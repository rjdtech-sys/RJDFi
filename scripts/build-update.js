...
const ALWAYS_EXCLUDE = [
    ...
    /^\.trae\//, /^\.qoder\//, /^\.vscode\//,
    /\.apk$/,
    /^firmware\/(?!.*\.bin$)/,  // Exclude firmware source, but INCLUDE .bin files
    /^scripts\//,
    /\.log$/, /\.tmp$/,
    '.gitignore',
    /\.md$/,
...
const INCLUDE_PATTERNS = [
    ...
    'firmware/NodeMCU_ESP8266/build/',  // Include compiled firmware .bin
];
...
async function uploadToSupabase(options) {
    ...
    if (!bucketExists) {
        if (listError) console.log(`   ⚠ Could not list buckets: ${listError.message}`);
        const { error: createError } = await supabase.storage.createBucket(bucketName, {
            public: false,
            file_size_limit: 104857600, // 100MB in bytes
        });
        if (createError) {
            // Bucket might have been created between check and create (race condition)
            if (createError.message?.includes('already exists') || createError.statusCode === 409) {
                console.log(`   ✅ Bucket already exists (created concurrently): ${bucketName}`);
            } else {
                console.error(`   ❌ Failed to create bucket:`, createError.message);
                // Try to continue anyway — bucket might exist
                console.log(`   ⚠ Attempting upload anyway...`);
            }
        } else {
    ...
    // Read release notes: CLI --notes > local update_release.json > generic fallback
    let resolvedNotes = notes;
    if (!resolvedNotes) {
        try {
            const localUpdateRelease = JSON.parse(fs.readFileSync(path.join(PROJECT_ROOT, 'update_release.json'), 'utf8'));
            if (localUpdateRelease.release_notes) {
                resolvedNotes = localUpdateRelease.release_notes;
                console.log(`   📝 Using release notes from local update_release.json`);
            }
        } catch {}
    }
    if (!resolvedNotes) {
        resolvedNotes = `System update v${versionName}`;
        console.log(`   ⚠ No release notes found, using generic fallback`);
    }
    ...
    if (promote) {
        ...
        // Read release notes for latest_release: CLI --notes > local latest_release.json > generic fallback
        let resolvedLatestNotes = notes;
        if (!resolvedLatestNotes) {
            try {
                const localLatestRelease = JSON.parse(fs.readFileSync(path.join(PROJECT_ROOT, 'latest_release.json'), 'utf8'));
                if (localLatestRelease.release_notes) {
                    resolvedLatestNotes = localLatestRelease.release_notes;
                    console.log(`   📝 Using release notes from local latest_release.json`);
                }
            } catch {}
        }
        if (!resolvedLatestNotes) {
            resolvedLatestNotes = `RJD PisoWiFi System v${versionName}`;
        }
        ...
        const latestRelease = {
            ...
            version_name: versionName,
            filename: '',
            release_notes: resolvedLatestNotes,
            published_at: new Date().toISOString(),
            bucket: bucketName,
        };
    ...
    // 4. Update version_history.json (keeps last 5 versions for rollback)
    console.log(`\n   [4/4] Updating version_history.json...`);
    try {
        // Download existing version_history.json
        let versionHistory = {};
        try {
            const { data: historyData, error: historyError } = await supabase.storage
                .from(bucketName)
                .download(`${folderPath}/version_history.json`);
            
            if (!historyError && historyData) {
                const text = await historyData.text();
                versionHistory = JSON.parse(text);
                console.log(`   📝 Loaded existing version history`);
            }
        } catch (e) {
            console.log(`   ℹ️  No existing version history, creating new one`);
        }

        // Add new version to history
        versionHistory[versionName] = {
            version_code: versionCode,
            version_name: versionName,
            filename: nxsFilename,
            release_notes: resolvedNotes,
        ...
        // Keep only last 5 versions (sort by version_code descending, keep top 5)
        const sortedVersions = Object.entries(versionHistory)
            .sort((a, b) => (b[1].version_code || 0) - (a[1].version_code || 0))
            .slice(0, 5);
        ...
        // Upload updated version_history.json
        const historyPath = `${folderPath}/version_history.json`;
        const { error: historyUploadError } = await supabase.storage
            .from(bucketName)
            .upload(historyPath, Buffer.from(JSON.stringify(trimmedHistory, null, 2)), {
                contentType: 'application/json',
                upsert: true,
            });

        if (historyUploadError) {
            console.error(`   ⚠  Failed to upload version_history.json:`, historyUploadError.message);
        } else {
            console.log(`   ✅ Uploaded: ${historyPath} (${Object.keys(trimmedHistory).length} versions)`);
        }
    } catch (e) {
        console.log(`   ⚠  Could not update version_history.json: ${e.message}`);
    }
