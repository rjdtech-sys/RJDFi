...
const { initGPIO, updateGPIO, registerSlotCallback, unregisterSlotCallback, setRelayState, pauseGPIO, resumeGPIO } = require('./lib/gpio');
const NodeMCUListener = require('./lib/nodemcu-listener');
const { getNodeMCULicenseManager } = require('./lib/nodemcu-license');
const network = require('./lib/network');
const multiwan = require('./lib/multiwan');
const wireguard = require('./lib/wireguard');

// ── GLOBAL SAFETY NET: Catch busboy/stream errors that escape middleware ─────
// All known busboy/multipart parse failures caused by truncated or malformed
// client uploads — benign, must never take the process down.
const BENIGN_UPLOAD_ERROR_PATTERNS = [
...
process.on('uncaughtException', (err) => {
  ...
  if (BENIGN_UPLOAD_ERROR_PATTERNS.some(p => msg.includes(p))) {
    console.warn('[UPLOAD] Busboy stream error caught at process level:', msg);
    return; // Swallow — malformed/truncated client request, not a real bug
  }
  console.error('[FATAL] Uncaught Exception:', err);
  process.exit(1); // Other uncaught errors = real bug, exit so PM2 restarts
});
...
function recordLoginFail(ip) {
  ...
  if (entry.count >= LOGIN_MAX_ATTEMPTS) {
    ...
    console.warn(`[SECURITY] Login locked for IP ${ip} — ${LOGIN_MAX_ATTEMPTS} failed attempts, locked for 15 min`);
  }
...
// Periodic cleanup of login rate limit map (every 10 min)
setInterval(() => {
  const now = Date.now();
  for (const [ip, entry] of _loginAttempts) {
    if (entry.lockedUntil && entry.lockedUntil <= now) _loginAttempts.delete(ip);
  }
}, 10 * 60 * 1000);

// ── SECURITY: Hardware API Key (for NodeMCU coin slot endpoints) ────────────
const HARDWARE_API_KEY = process.env.HARDWARE_API_KEY || crypto.randomBytes(16).toString('hex');
// NOTE: Set HARDWARE_API_KEY in .env for persistent key across restarts
// NodeMCU firmware should send this as X-Hardware-Key header
...
io.on('connection', (socket) => {
  ...
  // ─── GPIO On-Demand: pause/resume coin detection when client is waiting ───
  socket.on('gpio-resume', () => {
    socket._gpioHolding = true;
    _gpioActiveClients++;
    if (_gpioActiveClients === 1) {
      resumeGPIO();
      // Notify NodeMCU devices to start accepting coins
      _notifyNodeMCUListening(true).catch(() => {});
    }
  });

  socket.on('gpio-pause', () => {
    _gpioActiveClients = Math.max(0, _gpioActiveClients - 1);
    if (_gpioActiveClients === 0) {
      pauseGPIO();
      // Notify NodeMCU devices to stop accepting coins
      _notifyNodeMCUListening(false).catch(() => {});
    }
  });

  socket.on('disconnect', () => {
    // If this client was holding GPIO active, release it
    if (socket._gpioHolding) {
      socket._gpioHolding = false;
      _gpioActiveClients = Math.max(0, _gpioActiveClients - 1);
      if (_gpioActiveClients === 0) {
        pauseGPIO();
        _notifyNodeMCUListening(false).catch(() => {});
...
// ─── GPIO On-Demand: reference counter for active coin-listening clients ───
let _gpioActiveClients = 0;

/**
 * Notify all registered NodeMCU devices whether the server is listening for coins.
 * Sends HTTP GET to each device's /api/listening endpoint.
 */
async function _notifyNodeMCUListening(isListening) {
  try {
    const devices = await db.getAllNodeMCUDevices();
    if (!devices || devices.length === 0) return;
    const state = isListening ? 'true' : 'false';
    for (const device of devices) {
      if (!device.ipAddress || device.status !== 'accepted') continue;
      try {
        const http = require('http');
        await new Promise((resolve, reject) => {
          const req = http.get(`http://${device.ipAddress}/api/listening?state=${state}`, (res) => {
            res.resume(); // consume response
            resolve();
          });
          req.on('error', () => resolve()); // Don't reject on device unreachable
          req.setTimeout(2000, () => { req.destroy(); resolve(); });
        });
      } catch (_e) {}
    }
    console.log(`[GPIO] NodeMCU devices notified: listening=${state}`);
  } catch (e) {
    // Silent fail — NodeMCU notification is best-effort
  }
}
...
// ─── Coin Detection Log (ring buffer — last 100 events, persisted to DB) ───
const COIN_LOG_MAX = 100;
const coinLog = [];
let coinLogId = 0;

async function addCoinLogEntry(entry) {
  const e = { id: ++coinLogId, timestamp: Date.now(), ...entry };
  // Auto-resolve slot MAC address to NodeMCU device friendly name
  if (!e.slot_name && e.slot && typeof e.slot === 'string' && e.slot.includes(':')) {
    try {
      const device = await db.getNodeMCUDevice(e.slot);
      if (device && device.name) {
        e.slot_name = device.name;
      }
    } catch (_) {}
  }
  coinLog.push(e);
  if (coinLog.length > COIN_LOG_MAX) coinLog.shift();
  try { io.emit('coin-log-entry', e); } catch (_) {}
  // Persist to SQLite so it survives reboots
  try { db.addCoinLogDb(e).catch(() => {}); } catch (_) {}
  return e;
}

// Load recent coin log entries from DB on startup
async function loadCoinLogFromDb() {
  try {
    const rows = await db.getCoinLogDb(COIN_LOG_MAX);
    if (Array.isArray(rows) && rows.length > 0) {
      // Rows come back newest-first, reverse for oldest-first in memory
      ...
      coinLog.push(...entries);
      coinLogId = entries.length > 0 ? Math.max(...entries.map(e => e.id)) : 0;
      console.log(`[COIN-LOG] Loaded ${entries.length} entries from database`);
    } else {
      console.log('[COIN-LOG] No previous entries in database');
    }
  } catch (e) {
    console.error('[COIN-LOG] Failed to load from DB:', e.message);
  }
}
...
async function pushNodeMCUPinsToDevice(device, { coinPinGpio, relayPinGpio, debounceMs }) {
  if (!device?.ipAddress) {
  ...
  if (typeof debounceMs === 'number') {
    params.debounce = String(debounceMs);
  }
...
// File upload middleware (wrapped to prevent busboy crash on malformed multipart)
const _fileUploadMiddleware = fileUpload({
...
// Routes handled by multer further down. express-fileupload must NOT touch these,
// otherwise it consumes the multipart body stream and multer fails with
// "Unexpected end of form".
const MULTER_HANDLED_PATHS = [
  '/api/admin/upload-audio',
  '/api/system/restore',
  '/api/system/update'
];
const MULTER_HANDLED_REGEX = /^\/api\/nodemcu\/[^/]+\/update$/;

app.use((req, res, next) => {
  // Skip multer-handled routes so their body stream stays intact
  if (MULTER_HANDLED_PATHS.includes(req.path) || MULTER_HANDLED_REGEX.test(req.path)) {
    return next();
  }
  // Intercept busboy stream errors to prevent process crash on malformed multipart bodies
  let handled = false;
  const origEmit = req.emit.bind(req);
  req.emit = function(event, ...args) {
    if (event === 'error' && !handled) {
      handled = true;
      console.warn('[UPLOAD] Malformed multipart request ignored:', args[0]?.message || args[0]);
      return next();
    }
    return origEmit(event, ...args);
  };
  _fileUploadMiddleware(req, res, (err) => {
    req.emit = origEmit; // restore
    if (err && !handled) {
      handled = true;
      console.warn('[UPLOAD] Malformed multipart request ignored:', err.message);
      return next();
    }
    if (!handled) next();
  });
...
// Coin Detection Log API (serves from persistent DB)
app.get('/api/coin-log', requireAdmin, async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 100;
    const rows = await db.getCoinLogDb(limit);
    res.json(Array.isArray(rows) ? rows : []);
  } catch (e) {
    // Fallback to in-memory buffer if DB query fails
    res.json(coinLog.slice().reverse());
  }
});
...
app.post('/api/login', async (req, res) => {
  ...
  if (!rateCheck.allowed) {
    console.warn(`[SECURITY] Login attempt blocked for IP ${clientIp} — locked for ${rateCheck.retryAfter} min`);
    return res.status(429).json({ 
      error: `Too many failed attempts. Account locked for ${rateCheck.retryAfter} minutes. Try again later.` 
    });
  }
  ...
  try {
    ...
    if (verifyPassword(password, admin.salt, admin.password_hash)) {
      // ── SECURITY: Auto-upgrade password hash if using old iterations ──
      const { needsRehash, salt: newSalt, hash: newHash } = require('./lib/auth').verifyPasswordWithUpgrade(password, admin.salt, admin.password_hash);
      if (needsRehash) {
        await db.run('UPDATE admin SET password_hash = ?, salt = ? WHERE username = ?', [newHash, newSalt, username]);
        console.log(`[SECURITY] Password hash upgraded for user '${username}' (PBKDF2 → 100,000 iterations)`);
      }
...
app.post('/api/license/activate', async (req, res) => {
  ...
  try {
    ...
    if (result.success) {
      ...
      invalidateLicenseCache(); // Force fresh license check on next /api/whoami
...
// Avoids hitting Supabase on every /api/whoami poll (every 5s per client)
// License status rarely changes — cache for 24 hours, refreshes on every reboot (cache is in-memory)
...
async function getCachedLicenseStatus() {
  ...
  const verification = await licenseManager.verifyLicense();
  const trialStatus = await checkTrialStatus(systemHardwareId, verification);
  _licenseCache.data = verification;
  _licenseCache.trialData = trialStatus;
  _licenseCache.timestamp = now;
  return { verification, trialStatus };
}

function invalidateLicenseCache() {
  _licenseCache.timestamp = 0;
}

// ─── MAC Address Cache ───
// Avoids repeated /proc/net/arp reads + ip neigh shell exec on every request
const _macCache = new Map();
const MAC_CACHE_TTL = 30000; // 30 seconds

function getCachedMac(ip) {
  const entry = _macCache.get(ip);
  if (entry && (Date.now() - entry.ts) < MAC_CACHE_TTL) return entry.mac;
  return null; // null means "not cached, must resolve"
}

function setCachedMac(ip, mac) {
  _macCache.set(ip, { mac, ts: Date.now() });
}
...
async function getMacFromIp(ip) {
  ...
  // Check MAC cache first — avoids file I/O and shell exec on every request
...
// --- Device reachability helpers (for Online/Offline status in Device Management) ---
// wifi_devices.last_seen is only refreshed by manual scans / coin events, so a device
// that is actively connected (especially on VLAN interfaces) can be reported Offline.
// These helpers verify real reachability via the kernel ARP table with a cheap, cached
// /proc/net/arp read, plus a throttled single ping for active-session devices.
let _arpSnapshot = null;      // Map ip -> mac (upper)
let _arpSnapshotTs = 0;
const ARP_SNAPSHOT_TTL = 5000; // 5s cache so polling doesn't hammer the file

// Device-preserving variant — the guard sweep needs the interface so it never
// touches WAN-side entries (REJECTing the ISP router's MAC would kill ALL
// return traffic for every client).
let _arpSnapshotByDev = null; // Map ip -> { mac, dev }
let _arpSnapshotByDevTs = 0;

function getArpSnapshotByDevice() {
  const now = Date.now();
  if (_arpSnapshotByDev && (now - _arpSnapshotByDevTs) < ARP_SNAPSHOT_TTL) return _arpSnapshotByDev;
  const map = new Map();
  try {
    const arpData = fs.readFileSync('/proc/net/arp', 'utf8');
    const lines = arpData.split('\n');
    for (let i = 1; i < lines.length; i++) { // skip header
      ...
      if (parts.length >= 6 && parts[3] && parts[3] !== '00:00:00:00:00:00') {
        map.set(parts[0], { mac: parts[3].toUpperCase(), dev: parts[5] || '' });
      }
    }
  } catch (e) {}
  _arpSnapshotByDev = map;
  _arpSnapshotByDevTs = now;
  return map;
}

function getArpSnapshot() {
  const now = Date.now();
  if (_arpSnapshot && (now - _arpSnapshotTs) < ARP_SNAPSHOT_TTL) return _arpSnapshot;
  const map = new Map();
  try {
    const arpData = fs.readFileSync('/proc/net/arp', 'utf8');
    const lines = arpData.split('\n');
    for (let i = 1; i < lines.length; i++) { // skip header
      ...
      // Format: IP HW-type Flags HW-address Mask Device
      if (parts.length >= 6 && parts[3] && parts[3] !== '00:00:00:00:00:00') {
        map.set(parts[0], parts[3].toUpperCase());
      }
    }
  } catch (e) {}
  _arpSnapshot = map;
...
const _lastDeviceProbe = new Map(); // mac -> ts, throttles ping probes
const DEVICE_PROBE_COOLDOWN = 20000; // max one ping probe per device per 20s

async function probeDeviceReachable(ip) {
  try {
...
async function applyRewardsForPurchase(mac, clientIp, pesos) {
  try {
    ...
    // Get or create device record with pending_reward_amount
    let existing = await db.get('SELECT id, credit_pesos, credit_minutes, pending_reward_amount, total_rewards_earned FROM wifi_devices WHERE mac = ?', [mac]);
    
    if (!existing) {
      // Create device record if it doesn't exist
      const id = `device_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      await db.run(
        'INSERT INTO wifi_devices (id, mac, ip, hostname, interface, ssid, signal, connected_at, last_seen, is_active, custom_name, credit_pesos, credit_minutes, pending_reward_amount, total_rewards_earned, first_seen_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [id, mac, clientIp || '', '', '', '', 0, Date.now(), Date.now(), 0, '', 0, 0, 0, 0, new Date().toISOString()]
      );
      existing = await db.get('SELECT id, credit_pesos, credit_minutes, pending_reward_amount, total_rewards_earned FROM wifi_devices WHERE mac = ?', [mac]);
    }

    // Calculate rewards with remainder tracking
    const pending = (existing.pending_reward_amount || 0) + amount;
    const units = Math.floor(pending / threshold);
    const bonusPesos = units * rewardCredit;
    const newPending = pending % threshold;

    // Update device with new credits and pending amount
    await db.run(
      'UPDATE wifi_devices SET credit_pesos = credit_pesos + ?, pending_reward_amount = ?, total_rewards_earned = total_rewards_earned + ?, last_seen = ? WHERE id = ?',
      [bonusPesos, newPending, bonusPesos, Date.now(), existing.id]
    );

    if (bonusPesos > 0) {
      console.log(
        `[REWARDS] Granted bonus credit for ${mac} | ₱${bonusPesos} from ₱${amount} (pending: ₱${newPending})`
      );
    } else {
      console.log(
        `[REWARDS] Tracking pending for ${mac} | ₱${newPending}/₱${threshold} toward next reward`
      );
    }
  } catch (e) {
...
// ─── Referral / Affiliate Program Helpers ───
...
async function getOrCreateReferralCode(mac) {
  const existing = await db.get('SELECT code FROM referral_codes WHERE mac_address = ?', [mac]);
  if (existing) return existing.code;
  let code = generateReferralCode();
  // Ensure uniqueness
  let tries = 0;
  while (tries < 10) {
    const dup = await db.get('SELECT id FROM referral_codes WHERE code = ?', [code]);
    if (!dup) break;
    code = generateReferralCode();
    tries++;
  }
  await db.run('INSERT INTO referral_codes (mac_address, code) VALUES (?, ?)', [mac, code]);
  // Also store on wifi_devices for quick access
  await db.run('UPDATE wifi_devices SET referral_code = ? WHERE mac = ?', [code, mac]);
  return code;
}

async function getReferralConfig() {
  const defaults = { enabled: false, referrerPointsPerPesos: 20, refereeBonusMinutes: 5, minPesosToTrigger: 20 };
  try {
    const row = await db.get("SELECT value FROM config WHERE key = 'referral_config'");
...
async function awardReferrerPoints(payingMac, pesos) {
  try {
    if (!payingMac || !pesos || pesos <= 0) return;
    const cfg = await getReferralConfig();
    if (!cfg.enabled) return;

    // Check if this device was referred by someone
    const device = await db.get('SELECT referred_by FROM wifi_devices WHERE mac = ?', [payingMac]);
    if (!device || !device.referred_by) return;

    const referrerMac = device.referred_by;
    const minPesos = parseInt(cfg.minPesosToTrigger, 10) || 20;
    if (pesos < minPesos) return;

    const pointsPerPesos = parseInt(cfg.referrerPointsPerPesos, 10) || 20;
    const points = Math.floor(pesos / pointsPerPesos);
    if (points <= 0) return;

    // Award points to referrer (added to both referral_points and credit_pesos)
    await db.run(
      'UPDATE wifi_devices SET referral_points = referral_points + ?, credit_pesos = credit_pesos + ? WHERE mac = ?',
      [points, points, referrerMac]
    );

    // Log the event
    await db.run(
      'INSERT INTO referral_events (referrer_mac, referee_mac, pesos_spent, points_earned) VALUES (?, ?, ?, ?)',
      [referrerMac, payingMac, pesos, points]
    );

    console.log(`[REFERRAL] Awarded ${points} point(s) to ${referrerMac} from ${payingMac} (₱${pesos})`);
  } catch (e) {
    console.error('[REFERRAL] Failed to award referrer points:', e);
  }
}
...
// ─── Captive Portal Assistant (CNA) trigger helpers ─────────────────
// Some devices — notably Samsung Galaxy (One UI) and certain Android/iOS
// builds — only launch the "Sign in to Wi-Fi network" prompt when the
// connectivity probe returns an HTTP 302 redirect. A 200 response with an
// HTML body is silently treated as "connected, no internet", so no portal
// pops up. Redirecting is the reliable, cross-device trigger. We also send
// no-cache headers so devices never reuse a stale "Success" result and skip
// re-probing when they reconnect.
function setCaptiveNoCache(res) {
  res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
  res.setHeader('Pragma', 'no-cache');
  res.setHeader('Expires', '0');
}

function redirectToCaptivePortal(req, res) {
  setCaptiveNoCache(res);
  // Redirect to the gateway root, which serves the lightweight public portal.
  // Using the request Host guarantees the target resolves back to this
  // gateway (that is how the probe reached us in the first place) and the
  // root path is never a probe, so it cannot cause a redirect loop.
  const host = (req.headers.host || '').trim();
  const target = host ? `http://${host}/` : '/';
  return res.redirect(302, target);
}

app.get(['/generate_204', '/gen_204', '/hotspot-detect.html', '/library/test/success.html', '/connecttest.txt', '/ncsi.txt', '/redirect', '/success.txt', '/canonical.html'], (req, res, next) => {
  try {
...
app.post('/api/admin/upload-audio', requireAdmin, (req, res, next) => {
  // Wrap multer so upload errors (malformed multipart, size limit, wrong type)
  // return a clean JSON response instead of falling into the global error handler
  upload.single('audio')(req, res, (err) => {
    if (err) {
      console.warn('[AUDIO-UPLOAD] Rejected upload:', err.message);
...
async function tryRoamingAuthorize(mac, clientIp, sessionToken) {
  try {
    ...
    if (!activeSession) {
      ...
      if (expiredSession) {
        // Rate-limit log: only once per MAC per 2 minutes
        if (!isRecentlyBlocked(`log:roaming:${mac}`)) {
          console.log(`[AUTH] Roaming DENIED: ${mac} has expired session (0 time) — keeping blocked`);
          _blockedMacCache.set(`log:roaming:${mac}`, { ts: Date.now() });
        }
        return false;
      }
    }
...
// ─── Expired Device Block Cache ───
// Avoids re-running iptables DNS rules + DB queries on EVERY request from expired devices
// Once a MAC is blocked, skip redundant blocking for 120 seconds
const _blockedMacCache = new Map();
const BLOCKED_MAC_TTL = 900000; // 15 minutes — iptables rules persist, skip redundant re-application

function isRecentlyBlocked(mac) {
  const entry = _blockedMacCache.get(mac);
  if (entry && (Date.now() - entry.ts) < BLOCKED_MAC_TTL) return true;
  return false;
}

function markAsBlocked(mac) {
  _blockedMacCache.set(mac, { ts: Date.now() });
}

async function checkAndBlockExpired(mac, clientIp) {
  if (!mac) return false;

  // Fast path: if we already blocked this MAC recently, skip ALL expensive operations
  if (isRecentlyBlocked(mac)) return true;
  ...
  try {
    ...
    if (activeSession) {
      // Device has new active session — remove from blocked cache
      _blockedMacCache.delete(mac);
      return false;
    }

    const expired = await db.get(
      'SELECT mac, ip, expired_at FROM sessions WHERE mac = ? AND remaining_seconds <= 0',
      [mac]
    );
    if (expired) {
      // Ensure the device stays blocked (DNS redirect to portal)
      ...
      // Mark this MAC as recently blocked to skip redundant work on next requests
      markAsBlocked(mac);
      return true;
    }
  } catch (e) {}
...
// Helper: ensure a device with no active session has proper blocking rules
// Removes stale whitelist rules (PREROUTING ACCEPT, 8.8.8.8 DNS) from previous session
// Applies blockMAC rules (DNS redirect to portal)
...
// /gen_204 is the legacy Android probe path — same 204/redirect semantics.
app.get(['/generate_204', '/gen_204'], async (req, res) => {
  ...
  if (mac) {
    ...
    // No session, no roaming — ensure DNS redirect rules are correct
    await ensureBlockedNoSession(mac, clientIp);
  }
  
  // Not authorized — issue a 302 redirect to the portal. Samsung Galaxy /
  // One UI and some Android/iOS builds only launch the captive portal prompt
  // on a redirect; a 200 HTML body is silently treated as "no internet".
  return redirectToCaptivePortal(req, res);
});
...
app.get('/hotspot-detect.html', async (req, res) => {
  ...
  if (mac) {
    ...
    // No session — ensure stale whitelist rules are removed
    await ensureBlockedNoSession(mac, clientIp);
  }
...
// /redirect is the Windows NCSI fallback probe — a 302 to the portal is
// exactly what makes Windows open the sign-in browser.
app.get(['/connecttest.txt', '/redirect'], async (req, res) => {
  const clientIp = req.ip ? req.ip.replace('::ffff:', '') : '';
  const mac = await getMacFromIp(clientIp);
...
// /success.txt (Firefox detectportal) and /canonical.html (Ubuntu/Firefox)
app.get(['/success.txt', '/canonical.html'], async (req, res) => {
  const clientIp = req.ip ? req.ip.replace('::ffff:', '') : '';
  const mac = await getMacFromIp(clientIp);
...
app.use(async (req, res, next) => {
  ...
  // Check if this is a DNS-based captive portal probe. Cover the well-known
  // connectivity-check hosts for Apple (iOS/macOS), Samsung/Android and
  // Windows so their probes are handled no matter how DNS resolves them.
  const captiveHosts = [
    ...
    'clients3.google.com', 'clients1.google.com', 'www.google.com',
    'play.googleapis.com', 'connect.rom.miui.com',
  ...
  if (captiveHosts.includes(host)) {
    ...
    // Not authorized — 302 redirect so Samsung/iOS/Windows reliably launch the
    // captive portal (a 200 HTML body is silently treated as "no internet").
    return redirectToCaptivePortal(req, res);
  }
...
app.use(async (req, res, next) => {
  ...
  const portalProbes = [
    ...
    '/library/test/success.html', '/redirect', '/canonical.html'
  ];
  ...
  if (mac) {
    ...
    // CRITICAL: Device has NO active session — MUST ensure DNS redirect rules are correct
    // Remove stale whitelist rules from previous session, apply DNS blocking rules
    await ensureBlockedNoSession(mac, clientIp);
    
    // Also run checkAndBlockExpired for DB state sync (expired_at marking)
    if (await checkAndBlockExpired(mac, clientIp)) {
      if (!isRecentlyBlocked(`log:${mac}`)) {
        console.log(`[AUTH] EXPIRED: ${mac} has 0 time — forcing portal`);
        _blockedMacCache.set(`log:${mac}`, { ts: Date.now() });
      }
    }
    
    // No active session — force the captive portal to launch.
    // IMPORTANT: Do NOT return 204 here — that tells the OS "you have internet"
    // and dismisses the portal. Redirect probes (Samsung/iOS need a 302).
    if (isProbe) {
      return redirectToCaptivePortal(req, res);
    }
  ...
  if (isProbe) {
      // Unknown/unresolved device probe (e.g. MAC not yet in the ARP cache) —
      // 302 redirect so the captive portal launches instead of serving the
      // admin SPA, which would not render inside the CNA mini-browser.
      return redirectToCaptivePortal(req, res);
  }
...
app.get('/api/whoami', async (req, res) => {
  ...
  try {
    // Use cached license status — avoids Supabase network call every 5s per client
    ...
    if (isRevoked) {
       ...
       const nodemcuMacs = await db.getNodeMCUMacs();
  ...
  let vlanId = null;
  // Check VLAN cache first — avoids shell exec on every request
  const vlanCacheEntry = _macCache.get(`vlan:${clientIp}`);
  if (vlanCacheEntry && (Date.now() - vlanCacheEntry.ts) < MAC_CACHE_TTL) {
    vlanId = vlanCacheEntry.vlanId;
    ...
    try {
      ...
      // Cache the VLAN result
      _macCache.set(`vlan:${clientIp}`, { vlanId, ts: Date.now() });
    } catch (e) {
  ...
  try {
    if (token && mac) {
      ...
      if (canAttempt) {
        ...
        if (sessionByToken && sessionByToken.mac !== mac && sessionByToken.remaining_seconds > 0) {
  ...
  // Fetch boardType so portal knows whether to show Main Machine (GPIO boards) or hide it (x64_pc)
  let boardType = 'orange_pi'; // default to GPIO board
  try {
    const boardRow = await db.get('SELECT value FROM config WHERE key = ?', ['boardType']);
    if (boardRow && boardRow.value) boardType = boardRow.value;
  } catch (e) {}
...
app.post('/api/credits/add', async (req, res) => {
  try {
    // ── SECURITY: Hardware API Key check (soft — logs warning, doesn't block yet) ──
    const hwKey = req.headers['x-hardware-key'];
    if (!hwKey || hwKey !== HARDWARE_API_KEY) {
      console.warn(`[SECURITY] /api/credits/add called WITHOUT valid hardware key from IP: ${req.ip}`);
      // SOFT CHECK: Still allow for now (backward compat with old firmware)
      // TODO: After re-flashing all NodeMCUs with firmware that sends HARDWARE_API_KEY, change to hard block:
      // return res.status(403).json({ success: false, error: 'Unauthorized hardware' });
    }
    ...
    if (existing) {
      ...
      const id = `device_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      await db.run(
        'INSERT INTO wifi_devices (id, mac, ip, hostname, interface, ssid, signal, connected_at, last_seen, is_active, custom_name, credit_pesos, credit_minutes, first_seen_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [id, mac, clientIp, '', '', '', 0, Date.now(), Date.now(), 0, '', safePesos, safeMinutes, new Date().toISOString()]
      );
    }
    ...
    try {
      await awardReferrerPoints(mac, safePesos);
    } catch (e) {}
...
app.post('/api/credits/use', async (req, res) => {
  try {
    ...
    // RATE EXPIRATION: stamp validity window from the matched rate (same rules as coin insert)
    const creditRateExpirationMs = rate && Number(rate.expiration_hours) > 0
      ? Math.round(Number(rate.expiration_hours) * 3600 * 1000)
      : 0;
    if (creditRateExpirationMs > 0) {
      const nowMs = Date.now();
      await db.run(
        `UPDATE sessions
         SET rate_expires_at = MAX(CASE WHEN rate_expires_at > 0 THEN rate_expires_at ELSE ? END, ?) + ?
         WHERE mac = ?`,
        [nowMs, nowMs, creditRateExpirationMs, mac]
      );
      console.log(`[CREDIT] Rate expiration for ${mac}: window extended by ${rate.expiration_hours}h`);
    }
    ...
    try {
      ...
      console.error('[CREDIT] Failed to update DNS rules on useCredit:', e);
    }
    ...
    try {
      await awardReferrerPoints(mac, pesos);
    } catch (e) {}
...
app.post('/api/session/resume', async (req, res) => {
  try {
    ...
    // RATE EXPIRATION: if the rate validity window elapsed while paused, forfeit the time
    if (Number(session.rate_expires_at) > 0 && Number(session.rate_expires_at) <= Date.now()) {
      await db.run(
        'UPDATE sessions SET remaining_seconds = 0, rate_expires_at = NULL, expired_at = ?, is_paused = 0 WHERE token = ?',
        [Date.now(), token]
      );
      try { await network.blockMAC(session.mac, session.ip); } catch (e) {}
      console.log(`[Session] Resume denied — rate expiration elapsed for ${session.mac}`);
      return res.status(400).json({ success: false, message: 'This session time has expired (rate expiration reached). Please insert a coin to start again.' });
    }
...
app.post('/api/sessions/start', async (req, res) => {
  ...
  try {
    ...
    if (requestedSlot && typeof requestedSlot === 'string' && requestedSlot.includes(':')) {
       const nodeDevice = await db.getNodeMCUDevice(requestedSlot);
    ...
    if (requestedToken) {
      ...
      if (sessionByToken) {
        if (sessionByToken.mac === mac) {
          await db.run(
            'UPDATE sessions SET remaining_seconds = remaining_seconds + ?, total_paid = total_paid + ?, ip = ?, download_limit = ?, upload_limit = ?, expired_at = NULL WHERE token = ?',
          ...
          await db.run('DELETE FROM sessions WHERE mac = ?', [sessionByToken.mac]);
          await db.run(
            'INSERT INTO sessions (mac, ip, remaining_seconds, total_paid, connected_at, download_limit, upload_limit, token, pausable, rate_expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [
              mac,
              clientIp,
              ...
              sessionByToken.rate_expires_at || null
            ]
          );
          migratedOldMac = sessionByToken.mac;
          migratedOldIp = sessionByToken.ip;
        ...
        if (existingByMac) {
          ...
          await db.run(
            'UPDATE sessions SET remaining_seconds = remaining_seconds + ?, total_paid = total_paid + ?, ip = ?, download_limit = ?, upload_limit = ?, expired_at = NULL, token = ? WHERE mac = ?',
    ...
    if (!tokenToUse) {
      ...
      await db.run(
        'INSERT INTO sessions (mac, ip, remaining_seconds, total_paid, download_limit, upload_limit, token, pausable) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(mac) DO UPDATE SET remaining_seconds = remaining_seconds + ?, total_paid = total_paid + ?, ip = ?, download_limit = ?, upload_limit = ?, expired_at = NULL, token = ?',
    ...
    // RATE EXPIRATION: stamp the session's validity window from the purchased rate.
    // Time must be consumed before rate_expires_at — pausing freezes the countdown
    // but does NOT extend this window. A top-up within a live window extends the
    // window from its current end; a top-up after expiry starts a fresh window.
    // Rates WITHOUT expiration_hours leave the existing window untouched so a small
    // top-up can never void an active expiration.
    const rateExpirationMs = rate && Number(rate.expiration_hours) > 0
      ? Math.round(Number(rate.expiration_hours) * 3600 * 1000)
      : 0;
    if (rateExpirationMs > 0) {
      const nowMs = Date.now();
      await db.run(
        `UPDATE sessions
         SET rate_expires_at = MAX(CASE WHEN rate_expires_at > 0 THEN rate_expires_at ELSE ? END, ?) + ?
         WHERE mac = ?`,
        [nowMs, nowMs, rateExpirationMs, mac]
      );
      console.log(`[AUTH] Rate expiration for ${mac}: window extended by ${rate.expiration_hours}h`);
    }

    await network.whitelistMAC(mac, clientIp);
    // CRITICAL: Clear blocked cache so connectivity probes succeed immediately
    _blockedMacCache.delete(mac);
    _blockedMacCache.delete(`log:${mac}`);
    _blockedMacCache.delete(`log:roaming:${mac}`);
    if (migratedOldMac && migratedOldIp) {
      await network.blockMAC(migratedOldMac, migratedOldIp);
    }
    
    console.log(`[AUTH] Session started for ${mac} (${clientIp}) - ${seconds}s, ₱${pesos}, Rate limits: ${downloadLimit}/${uploadLimit} Mbps (QoS from config defaults)`);
    console.log(`[AUTH] New user connected: MAC=${mac} | Session ID=${tokenToUse}`);
    
    // Log session crediting
    addCoinLogEntry({ type: 'session_credit', event: 'Session Credited', mac, ip: clientIp, token: tokenToUse, amount: pesos, seconds, minutes, slot: requestedSlot || 'main' });
...
app.post('/api/sessions/restore', async (req, res) => {
  ...
  try {
    ...
    await db.run(
      'INSERT INTO sessions (mac, ip, remaining_seconds, total_paid, connected_at, download_limit, upload_limit, token, rate_expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [mac, clientIp, session.remaining_seconds + extraTime, session.total_paid + extraPaid, session.connected_at, session.download_limit, session.upload_limit, token, session.rate_expires_at || null]
    );
...
app.post('/api/sessions/resume', async (req, res) => {
  ...
  try {
    ...
    // RATE EXPIRATION: if the rate validity window elapsed while paused, forfeit the time
    if ((session.remaining_seconds || 0) > 0 && Number(session.rate_expires_at) > 0 && Number(session.rate_expires_at) <= Date.now()) {
      await db.run(
        'UPDATE sessions SET remaining_seconds = 0, rate_expires_at = NULL, expired_at = ?, is_paused = 0 WHERE token = ?',
        [Date.now(), token]
      );
      try { await network.blockMAC(session.mac, session.ip); } catch (e) {}
      console.log(`[AUTH] Resume denied — rate expiration elapsed for ${session.mac}`);
      return res.status(400).json({ error: 'This session time has expired (rate expiration reached). Please insert a coin to start again.' });
    }
    ...
    // CRITICAL: Clear blocked cache so connectivity probes succeed immediately
    _blockedMacCache.delete(session.mac);
    _blockedMacCache.delete(`log:${session.mac}`);
    ...
    try {
      ...
      if (lan) {
        ...
        for (const [mac, ip] of devicesToRestore) {
          // whitelistMAC applies DNS bypass rules and Traffic Control limits
...
// Get or create referral code for current client
app.get('/api/referral/code', async (req, res) => {
  try {
    ...
    const mac = await getMacFromIp(clientIp);
    if (!mac) return res.json({ enabled: true, code: null, eligible: false });

    // Check if device has spent at least ₱20 (from sessions total_paid or wifi_devices credit history)
    const session = await db.get(
      'SELECT COALESCE(SUM(total_paid), 0) as totalSpent FROM sessions WHERE mac = ?',
      [mac]
    );
    const totalSpent = session?.totalSpent || 0;
    
    // Device must have spent at least ₱20 to have a referral code
    const minSpend = 20;
    if (totalSpent < minSpend) {
      return res.json({ enabled: true, code: null, eligible: false, totalSpent, minSpend });
    }

    const code = await getOrCreateReferralCode(mac);
    res.json({ enabled: true, code, eligible: true, totalSpent });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/referral/redeem', async (req, res) => {
  try {
    const cfg = await getReferralConfig();
    if (!cfg.enabled) return res.status(400).json({ error: 'Referral program is disabled.' });
    ...
    const clientIp = req.headers['x-forwarded-for'] || req.socket.remoteAddress || req.ip;
    const mac = await getMacFromIp(clientIp);
    if (!mac) return res.status(400).json({ error: 'Could not identify your device.' });

    // Find the referrer
    const referrerRow = await db.get('SELECT mac_address FROM referral_codes WHERE code = ?', [code.toUpperCase()]);
    if (!referrerRow) return res.status(400).json({ error: 'Invalid referral code.' });

    const referrerMac = referrerRow.mac_address;

    // Self-referral check
    if (referrerMac === mac) return res.status(400).json({ error: 'You cannot use your own referral code.' });

    // Check if already referred
    const device = await db.get('SELECT referred_by, first_seen_at FROM wifi_devices WHERE mac = ?', [mac]);
    if (device && device.referred_by) return res.status(400).json({ error: 'You have already used a referral code.' });

    // Existing customer check: if device was first seen before the referrer's code was created, block
    const referrerCodeRow = await db.get('SELECT created_at FROM referral_codes WHERE mac_address = ?', [referrerMac]);
    if (device && device.first_seen_at && referrerCodeRow) {
      const firstSeen = new Date(device.first_seen_at).getTime();
      const codeCreated = new Date(referrerCodeRow.created_at).getTime();
      if (firstSeen < codeCreated) {
        return res.status(400).json({ error: 'Your device was registered before this referral code was created. Only new customers can use referral codes.' });
      }
    }

    const bonusMinutes = parseInt(cfg.refereeBonusMinutes, 10) || 5;

    // Update the referee device
    if (device) {
      await db.run(
        'UPDATE wifi_devices SET referred_by = ?, credit_minutes = credit_minutes + ? WHERE mac = ?',
        [referrerMac, bonusMinutes, mac]
      );
    } else {
      // Device doesn't exist yet - will be linked when they first connect
      const id = `device_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      await db.run(
        'INSERT INTO wifi_devices (id, mac, ip, hostname, interface, ssid, signal, connected_at, last_seen, is_active, custom_name, credit_pesos, credit_minutes, referred_by, first_seen_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [id, mac, clientIp || '', '', '', '', 0, Date.now(), Date.now(), 0, '', 0, bonusMinutes, referrerMac, new Date().toISOString()]
      );
    }

    // Log the referral event
    await db.run(
      'INSERT INTO referral_events (referrer_mac, referee_mac, referee_ip, referral_code, referee_bonus_minutes) VALUES (?, ?, ?, ?, ?)',
      [referrerMac, mac, clientIp, code.toUpperCase(), bonusMinutes]
    );

    console.log(`[REFERRAL] ${mac} redeemed code ${code.toUpperCase()} from ${referrerMac} → ${bonusMinutes} bonus minutes`);

    res.json({ success: true, bonusMinutes });
  } catch (err) {
...
app.get('/api/referral/config', requireAdmin, async (req, res) => {
  try {
    const cfg = await getReferralConfig();
    res.json(cfg);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
// Save referral config (admin)
app.post('/api/referral/config', requireAdmin, async (req, res) => {
  try {
    const body = req.body || {};
    const enabled = !!body.enabled;
    const referrerPointsPerPesos = parseInt(body.referrerPointsPerPesos, 10);
    const refereeBonusMinutes = parseInt(body.refereeBonusMinutes, 10);
    const minPesosToTrigger = parseInt(body.minPesosToTrigger, 10);

    if (!referrerPointsPerPesos || referrerPointsPerPesos <= 0 || isNaN(refereeBonusMinutes) || refereeBonusMinutes < 0 || !minPesosToTrigger || minPesosToTrigger <= 0) {
      return res.status(400).json({ error: 'Invalid referral configuration.' });
    }

    const payload = JSON.stringify({ enabled, referrerPointsPerPesos, refereeBonusMinutes, minPesosToTrigger });
    await db.run(
      "INSERT INTO config (key, value) VALUES ('referral_config', ?) ON CONFLICT(key) DO UPDATE SET value = ?",
      [payload, payload]
    );
...
app.get('/api/referral/stats', requireAdmin, async (req, res) => {
  try {
    const totalReferrals = await db.get('SELECT COUNT(*) as count FROM referral_events WHERE referee_bonus_minutes > 0');
    const totalPoints = await db.get('SELECT COALESCE(SUM(points_earned), 0) as total FROM referral_events');
    const topReferrers = await db.all(`
      SELECT rc.mac_address, rc.code, 
        COALESCE(SUM(re.points_earned), 0) as points,
        COUNT(DISTINCT re.referee_mac) as referral_count
      FROM referral_codes rc
      LEFT JOIN referral_events re ON re.referrer_mac = rc.mac_address
      GROUP BY rc.mac_address, rc.code
      ORDER BY points DESC
      LIMIT 10
    `);
    const recentEvents = await db.all('SELECT * FROM referral_events ORDER BY created_at DESC LIMIT 50');
...
// Get per-referrer stats
app.get('/api/referral/stats/:mac', requireAdmin, async (req, res) => {
  try {
    const { mac } = req.params;
    const referrer = await db.get('SELECT * FROM wifi_devices WHERE mac = ?', [mac]);
    const events = await db.all('SELECT * FROM referral_events WHERE referrer_mac = ? ORDER BY created_at DESC', [mac]);
    const totalPoints = await db.get('SELECT COALESCE(SUM(points_earned), 0) as total FROM referral_events WHERE referrer_mac = ?', [mac]);
    const uniqueReferees = await db.get('SELECT COUNT(DISTINCT referee_mac) as count FROM referral_events WHERE referrer_mac = ?', [mac]);

    res.json({
      referrer,
      events,
      totalPoints: totalPoints?.total || 0,
      uniqueReferees: uniqueReferees?.count || 0
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
// Get game balance (player)
app.get('/api/game/balance', async (req, res) => {
  try {
    const token = getSessionToken(req);
    if (!token) return res.json({ credits: 0 });
    
    const player = await db.get(
      'SELECT credits FROM player_credits WHERE token = ?',
      [token]
    );
    
    res.json({ credits: player?.credits || 0 });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// Spin the wheel (player)
app.post('/api/game/spin', async (req, res) => {
  try {
    ...
    const enabledRow = await db.get("SELECT value FROM config WHERE key = 'game_enabled'");
    if (!enabledRow || enabledRow.value !== 'true') {
      return res.status(403).json({ error: 'Game is not enabled' });
    }

    // Load game config
    const costRow = await db.get("SELECT value FROM config WHERE key = 'game_cost_per_spin'");
    const cooldownRow = await db.get("SELECT value FROM config WHERE key = 'game_cooldown_ms'");
    const dailyLimitRow = await db.get("SELECT value FROM config WHERE key = 'game_daily_spin_limit'");
    const segmentsRow = await db.get("SELECT value FROM config WHERE key = 'game_segments'");

    const cost = parseInt(costRow?.value || '5', 10);
    const cooldownMs = parseInt(cooldownRow?.value || '300000', 10);
    const dailyLimit = parseInt(dailyLimitRow?.value || '20', 10);
    const segments = JSON.parse(segmentsRow?.value || '[]');

    // Get or create player record
    let player = await db.get('SELECT * FROM player_credits WHERE token = ?', [token]);
    const now = new Date();
    const today = now.toISOString().split('T')[0];

    if (!player) {
      await db.run(
        'INSERT INTO player_credits (token, credits, total_earned, total_redeemed, spins_today, spins_date) VALUES (?, 0, 0, 0, 0, ?)',
        [token, today]
      );
      player = await db.get('SELECT * FROM player_credits WHERE token = ?', [token]);
    }

    // Reset daily spins if new day
    if (player.spins_date !== today) {
      await db.run(
        'UPDATE player_credits SET spins_today = 0, spins_date = ? WHERE token = ?',
        [today, token]
      );
      player.spins_today = 0;
      player.spins_date = today;
    }

    // Check cooldown
    if (player.last_spin_at) {
      const lastSpin = new Date(player.last_spin_at);
    ...
    // Check daily limit
    if (player.spins_today >= dailyLimit) {
      return res.status(429).json({ error: 'Daily spin limit reached' });
    }

    // Check balance
    if (player.credits < cost) {
      return res.status(400).json({ error: 'Insufficient credits', required: cost, balance: player.credits });
    }

    // Deduct cost
    await db.run(
      'UPDATE player_credits SET credits = credits - ? WHERE token = ?',
      [cost, token]
    );
    ...
    let cumulative = 0;
    let prize = segments[segments.length - 1]; // Fallback
    for (const segment of segments) {
      cumulative += (segment.weight || 0);
    ...
    // Add prize to credits (if > 0)
    if (prizeValue > 0) {
      await db.run(
        'UPDATE player_credits SET credits = credits + ?, total_earned = total_earned + ? WHERE token = ?',
        [prizeValue, prizeValue, token]
      );
    }

    // Update spin tracking
    await db.run(
      'UPDATE player_credits SET last_spin_at = ?, spins_today = spins_today + 1 WHERE token = ?',
      [now.toISOString(), token]
    );

    // Log the spin
    await db.run(
      'INSERT INTO game_spins (token, prize_value, prize_label, cost) VALUES (?, ?, ?, ?)',
      [token, prizeValue, prize.label || '', cost]
    );

    res.json({
      success: true,
      prize: {
        label: prize.label || 'Try Again',
        value: prizeValue
      },
      new_balance: player.credits - cost + prizeValue
    });
  } catch (err) {
...
// Redeem credits for time (player)
app.post('/api/game/redeem', async (req, res) => {
  try {
    ...
    // Check balance
    const player = await db.get('SELECT credits FROM player_credits WHERE token = ?', [token]);
    if (!player || player.credits < redeemAmount) {
      return res.status(400).json({ error: 'Insufficient credits' });
    }

    // Look up rate from rates table
    let rate = await db.get('SELECT interval_minutes FROM rates WHERE pesos = ?', [redeemAmount]);
    
    let minutes;
    if (rate) {
      minutes = rate.interval_minutes;
    } else {
      // No exact match, use base rate
      const baseRate = await db.get('SELECT interval_minutes, pesos FROM rates ORDER BY pesos ASC LIMIT 1');
      if (!baseRate) {
        return res.status(500).json({ error: 'No rates configured' });
      }
      minutes = Math.floor((redeemAmount / baseRate.pesos) * baseRate.interval_minutes);
    }
    ...
    // Get MAC for this token
    const session = await db.get('SELECT mac, ip FROM sessions WHERE token = ? AND remaining_seconds > 0', [token]);
    if (!session) {
    ...
    const seconds = minutes * 60;
    await db.run(
      'UPDATE sessions SET remaining_seconds = remaining_seconds + ?, expired_at = NULL WHERE token = ?',
      [seconds, token]
    );

    // Deduct credits
    await db.run(
      'UPDATE player_credits SET credits = credits - ?, total_redeemed = total_redeemed + ? WHERE token = ?',
      [redeemAmount, redeemAmount, token]
    );
    ...
    try {
      ...
      console.error('[GAME] Failed to whitelist MAC:', e);
    }

    res.json({
      success: true,
      minutes_added: minutes,
      remaining_credits: player.credits - redeemAmount
    });
  } catch (err) {
...
app.get('/api/game/settings', requireAdmin, async (req, res) => {
  try {
    ...
    const enabled = await getConfig('game_enabled', 'false');
    const costPerSpin = await getConfig('game_cost_per_spin', '5');
    const cooldownMs = await getConfig('game_cooldown_ms', '300000');
    const dailyLimit = await getConfig('game_daily_spin_limit', '20');
    const segments = await getConfig('game_segments', '[]');

    // Get rewards config too
    const rewardsRow = await db.get("SELECT value FROM config WHERE key = 'rewards_config'");
    let rewards = { enabled: false, thresholdPesos: 20, rewardCreditPesos: 1 };
    if (rewardsRow?.value) {
      try {
        rewards = JSON.parse(rewardsRow.value);
      } catch (e) {}
    }

    res.json({
      game_enabled: enabled === 'true',
      game_cost_per_spin: parseInt(costPerSpin, 10),
      game_cooldown_ms: parseInt(cooldownMs, 10),
      game_daily_spin_limit: parseInt(dailyLimit, 10),
      game_segments: JSON.parse(segments),
      rewards_enabled: rewards.enabled,
      rewards_threshold: rewards.thresholdPesos,
      rewards_credit: rewards.rewardCreditPesos
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// Save game settings (admin)
app.post('/api/game/settings', requireAdmin, async (req, res) => {
  try {
    const body = req.body || {};

    // Only update game settings if game fields are present
    const hasGameFields = body.game_enabled !== undefined || body.game_cost_per_spin !== undefined || 
                          body.game_cooldown_ms !== undefined || body.game_daily_spin_limit !== undefined ||
                          body.game_segments !== undefined;

    if (hasGameFields) {
      const updates = [
        ['game_enabled', body.game_enabled ? 'true' : 'false'],
        ['game_cost_per_spin', String(parseInt(body.game_cost_per_spin, 10) || 5)],
        ['game_cooldown_ms', String(parseInt(body.game_cooldown_ms, 10) || 300000)],
        ['game_daily_spin_limit', String(parseInt(body.game_daily_spin_limit, 10) || 20)],
        ['game_segments', JSON.stringify(body.game_segments || [])]
      ];
    ...
    // Only update rewards settings if rewards fields are present
    const hasRewardsFields = body.rewards_enabled !== undefined || body.rewards_threshold !== undefined || 
                             body.rewards_credit !== undefined;

    if (hasRewardsFields) {
      // Read existing rewards config to fill in any missing fields
      let existing = { enabled: false, thresholdPesos: 20, rewardCreditPesos: 1 };
      try {
        const row = await db.get("SELECT value FROM config WHERE key = 'rewards_config'");
      ...
      const rewardsPayload = JSON.stringify({
        enabled: body.rewards_enabled !== undefined ? !!body.rewards_enabled : existing.enabled,
        thresholdPesos: body.rewards_threshold !== undefined ? (parseInt(body.rewards_threshold, 10) || 20) : existing.thresholdPesos,
        rewardCreditPesos: body.rewards_credit !== undefined ? (parseInt(body.rewards_credit, 10) || 1) : existing.rewardCreditPesos
      });
...
app.get('/api/game/stats', requireAdmin, async (req, res) => {
  try {
    const today = new Date().toISOString().split('T')[0];

    // Total spins today
    const spinsToday = await db.get(
      "SELECT COUNT(*) as count, COALESCE(SUM(prize_value), 0) as total_awarded FROM game_spins WHERE DATE(spun_at) = ?",
      [today]
    );

    // Total redeemed today (from player_credits)
    const redeemedToday = await db.get(
      "SELECT COALESCE(SUM(total_redeemed), 0) as total FROM player_credits WHERE DATE(updated_at) = ?",
      [today]
    );

    // Active players (played in last 7 days)
    const activePlayers = await db.get(
      "SELECT COUNT(*) as count FROM player_credits WHERE last_spin_at > DATETIME('now', '-7 days')"
    );

    // Recent spins
    const recentSpins = await db.all(
      "SELECT token, prize_value, prize_label, cost, spun_at FROM game_spins ORDER BY spun_at DESC LIMIT 20"
    );

    res.json({
      total_spins_today: spinsToday.count || 0,
      total_credits_awarded_today: spinsToday.total_awarded || 0,
      total_credits_redeemed_today: redeemedToday.total || 0,
...
app.get('/api/config', requireAdmin, async (req, res) => {
  try {
    ...
    const nodemcuDevicesList = await db.getAllNodeMCUDevices();
    ...
    const boardTypeValue = board?.value || 'x64_pc';
    const coinPinValue = (boardTypeValue === 'x64_pc' || boardTypeValue === 'none') ? null : parseInt(pin?.value || '2');
    res.json({ 
      boardType: boardTypeValue, 
      coinPin: coinPinValue,
      ...
      nodemcuDevices: nodemcuDevicesList,
...
app.post('/api/config', requireAdmin, async (req, res) => {
  try {
    ...
    const coinPinDbValue = (req.body.boardType === 'x64_pc' || req.body.boardType === 'none') ? '' : String(req.body.coinPin ?? '');
    await db.run('INSERT OR REPLACE INTO config (key, value) VALUES (?, ?)', ['coinPin', coinPinDbValue]);
    ...
    // Handle multi-NodeMCU devices — write to table instead of JSON blob
    if (req.body.nodemcuDevices !== undefined && Array.isArray(req.body.nodemcuDevices)) {
      for (const device of req.body.nodemcuDevices) {
        await db.upsertNodeMCUDevice(device);
      }
      // Also keep the old config key in sync for backward compatibility
...
// Rate-limit registration to prevent CPU spikes from firmware bug
// (unaccepted/unlicensed devices hammer this endpoint every loop iteration)
const _nodemcuRegRateLimit = new Map(); // mac -> last request timestamp
const NODEMCU_REG_MIN_INTERVAL = 10000; // 10 seconds minimum between registrations per MAC

app.post('/api/nodemcu/register', async (req, res) => {
  try {
    const { macAddress, ipAddress, authenticationKey, firmwareVersion } = req.body;
    ...
    // Rate limit: skip if this MAC registered too recently
    const now = Date.now();
    const lastReg = _nodemcuRegRateLimit.get(macAddress) || 0;
    if ((now - lastReg) < NODEMCU_REG_MIN_INTERVAL) {
      // Return cached-like response without doing any work
      return res.json({ success: true, device: null, licensed: false, rate_limited: true, message: 'Too frequent — retry later' });
    }
    _nodemcuRegRateLimit.set(macAddress, now);

    // Periodic cleanup of rate limit map
    if (_nodemcuRegRateLimit.size > 100) {
      for (const [mac, ts] of _nodemcuRegRateLimit) {
        if (now - ts > NODEMCU_REG_MIN_INTERVAL * 3) _nodemcuRegRateLimit.delete(mac);
      }
    }
    ...
    const existingDevice = await db.getNodeMCUDevice(macAddress);

    // Auto-detect VLAN from NodeMCU IP address (same logic as /api/whoami)
    let detectedVlanId = null;
    try {
      const { stdout } = await execPromise(`ip route get ${ipAddress}`);
      const match = stdout.match(/dev\s+(\S+)/);
      if (match && match[1]) {
        const iface = match[1];
        const vlanMatch = iface.match(/\.([0-9]+)$/);
        if (vlanMatch) {
          detectedVlanId = parseInt(vlanMatch[1], 10);
        }
      }
    } catch (e) {
      // Ignore route errors (device may not be reachable yet)
    }

    if (existingDevice) {
       // Update existing device info (e.g. IP might have changed)
       const now = new Date().toISOString();
       const updateFields = { ipAddress, lastSeen: now };
       if (firmwareVersion) updateFields.firmwareVersion = firmwareVersion;
       // Auto-update VLAN if detected and different from stored value
       if (detectedVlanId !== null && detectedVlanId !== existingDevice.vlanId) {
         updateFields.vlanId = detectedVlanId;
         existingDevice.vlanId = detectedVlanId;
         console.log(`[NODEMCU] VLAN auto-updated: ${existingDevice.name} → VLAN ${detectedVlanId} (from IP ${ipAddress})`);
       }
       await db.updateNodeMCUDevice(macAddress, updateFields);
       existingDevice.ipAddress = ipAddress;
       existingDevice.lastSeen = now;
       if (firmwareVersion) existingDevice.firmwareVersion = firmwareVersion;
       
       console.log(`[NODEMCU] Device Heartbeat | Name: ${existingDevice.name} | IP: ${ipAddress} | Status: ${existingDevice.status}`);

       // Sync heartbeat to cloud immediately
       edgeSync.syncNodeMCUDevice(existingDevice).catch(e => console.error('[NODEMCU] Failed to sync heartbeat:', e));
    ...
    const newDevice = {
      ...
      relayPin: 14,
      status: 'pending',
      vlanId: detectedVlanId !== null ? detectedVlanId : 13,
    ...
    // Insert into table
    await db.upsertNodeMCUDevice(newDevice);
...
app.post('/api/nodemcu/authenticate', async (req, res) => {
  try {
    ...
    const device = await db.getNodeMCUDevice(macAddress);
    ...
    const now = new Date().toISOString();
    await db.updateNodeMCUDevice(macAddress, { lastSeen: now });
...
const deviceHealthTimer = setInterval(async () => {
  try {
    ...
    const OFFLINE_THRESHOLD = 60000; // 60s threshold for offline detection

    for (const device of devices) {
      if (device.status !== 'accepted') continue;
...
app.post('/api/nodemcu/pulse', async (req, res) => {
  try {
    ...
    console.log(`[NODEMCU PULSE #${requestId}] IN: MAC=${macAddress} denom=${denomination} slot=${slotId}`);

    // Find device by MAC address (case-insensitive)
    const device = await db.getNodeMCUDevice(macAddress);
    
    if (!device || device.status !== 'accepted') {
      console.log(`[NODEMCU PULSE #${requestId}] REJECTED: device not found/not accepted`);
      return res.status(403).json({ error: 'Device not authorized' });
    }

    const licenseStatus = await nodeMCULicenseManager.verifyLicense(macAddress);
    if (!licenseStatus || licenseStatus.isValid !== true) {
      console.log(`[NODEMCU PULSE #${requestId}] REJECTED: license invalid`);
      return res.status(403).json({
        error: 'YOUR COINSLOT MACHINE IS DISABLED',
        frozen: true,
        licenseType: licenseStatus?.licenseType || null,
    ...
    const prevPulses = device.totalPulses || 0;
    const prevRevenue = device.totalRevenue || 0;

    // Update device stats
    await db.updateNodeMCUDevice(macAddress, {
      totalPulses: prevPulses + denomination,
      totalRevenue: prevRevenue + denomination,
      lastSeen: new Date().toISOString()
    });

    console.log(`[NODEMCU PULSE #${requestId}] OK: ${device.name} | pulses ${prevPulses}→${prevPulses + denomination} | revenue ${prevRevenue}→${prevRevenue + denomination}`);
...
app.post('/api/nodemcu/:deviceId/status', requireAdmin, async (req, res) => {
  try {
    ...
    // Find device by ID
    const device = await db.getNodeMCUDeviceById(deviceId);
    if (!device) {
      return res.status(404).json({ error: 'Device not found' });
    }
    ...
    if (vlanId) updateFields.vlanId = parseInt(vlanId);
    
    await db.updateNodeMCUDevice(device.macAddress, updateFields);
    
    const updatedDevice = await db.getNodeMCUDeviceById(deviceId);
...
app.post('/api/nodemcu/:deviceId/rates', requireAdmin, async (req, res) => {
  try {
    ...
    await db.updateNodeMCUDevice(device.macAddress, { rates });
...
app.post('/api/nodemcu/:deviceId/coinsout', requireAdmin, async (req, res) => {
  try {
    ...
    await db.updateNodeMCUDevice(device.macAddress, {
      totalRevenue: 0,
      lastCoinsOutGross: gross,
      lastCoinsOutNet: net,
      lastCoinsOutDate: date || new Date().toISOString()
...
app.get('/api/nodemcu/available', async (req, res) => {
  try {
    ...
    // VLAN isolation: vlanId query param is REQUIRED. No VLAN = no devices returned (strict isolation)
    const requestedVlanId = req.query.vlanId !== undefined ? parseInt(req.query.vlanId, 10) : null;
    if (requestedVlanId === null || isNaN(requestedVlanId)) {
      console.log('[NodeMCU Available] No VLAN ID provided, returning empty');
      return res.json([]);
    }
    
    console.log(`[NodeMCU Available] Request for VLAN ${requestedVlanId}, total devices: ${devices.length}`);
    devices.forEach(d => console.log(`  - ${d.name} (${d.macAddress}): vlanId=${d.vlanId}, status=${d.status}, isOnline=${(Date.now() - new Date(d.lastSeen).getTime()) < 15000}`));
    
    // Filter only accepted devices and calculate online status
    const now = new Date().getTime();
    const licenseManager = getNodeMCULicenseManager();

    // Step 1: accepted devices matching by stored vlanId
    const vlanMatchedDevices = devices.filter(d => d.status === 'accepted' && d.vlanId == requestedVlanId);

    // Step 2: IP-based fallback — accepted devices whose stored vlanId doesn't match
    // but whose IP actually routes through the requested VLAN interface
    const vlanIfaceName = `br-lan.${requestedVlanId}`;
    const unmatchedAccepted = devices.filter(d => d.status === 'accepted' && d.vlanId != requestedVlanId && d.ipAddress);
    const ipMatchedDevices = [];
    for (const d of unmatchedAccepted) {
      try {
        const { stdout } = await execPromise(`ip route get ${d.ipAddress}`);
        const m = stdout.match(/dev\s+(\S+)/);
        if (m && m[1] === vlanIfaceName) {
          console.log(`[NodeMCU Available] IP-based VLAN match: ${d.name} (${d.ipAddress}) → ${vlanIfaceName} (stored vlanId=${d.vlanId}, actual=${requestedVlanId})`);
          ipMatchedDevices.push(d);
          // Auto-fix: update stored vlanId for next time
          db.updateNodeMCUDevice(d.macAddress, { vlanId: requestedVlanId }).catch(() => {});
        }
      } catch (_e) {}
    }

    // Combine both match sets
    const matchedDevices = [...vlanMatchedDevices, ...ipMatchedDevices];

    const availableDevices = await Promise.all(matchedDevices
      .map(async d => {
    ...
    console.log(`[NodeMCU Available] Returning ${availableDevices.length} devices for VLAN ${requestedVlanId}`);
...
app.get('/api/nodemcu/status/:mac', async (req, res) => {
  try {
    const device = await db.getNodeMCUDevice(req.params.mac);
...
app.post('/api/nodemcu/:deviceId/config', requireAdmin, async (req, res) => {
  try {
    const { deviceId } = req.params;
    const { name, vlanId, pin, coinPinLabel, coinPin, relayPinLabel, relayPin, debounceMs } = req.body;
    
    const previousDevice = await db.getNodeMCUDeviceById(deviceId);
    if (!previousDevice) {
      return res.status(404).json({ error: 'Device not found' });
    }
    ...
    const updatedDeviceData = {
      ...
      debounceMs: typeof debounceMs === 'number' && debounceMs >= 10 && debounceMs <= 500 ? debounceMs : (previousDevice.debounceMs ?? 50)
    };
    
    await db.updateNodeMCUDevice(previousDevice.macAddress, {
      name: updatedDeviceData.name,
      vlanId: updatedDeviceData.vlanId,
      pin: nextCoinGpio,
      coinPin: nextCoinGpio,
      coinPinLabel: nextCoinLabel,
      relayPin: nextRelayGpio,
      relayPinLabel: nextRelayLabel,
      debounceMs: updatedDeviceData.debounceMs
    });
    ...
    const debounceChanged = typeof debounceMs === 'number' && debounceMs !== (previousDevice.debounceMs ?? 50);

    let deviceApply = null;
    if (pinsChanged || debounceChanged) {
      deviceApply = await pushNodeMCUPinsToDevice(updatedDeviceData, {
    ...
    res.json({ success: true, device: updatedDeviceData, applied: deviceApply });
  } catch (err) {
...
app.delete('/api/nodemcu/:deviceId', requireAdmin, async (req, res) => {
  try {
    ...
    await db.deleteNodeMCUDevice(device.macAddress);
...
app.post('/api/nodemcu/:deviceId/update', requireAdmin, uploadFirmware.single('firmware'), async (req, res) => {
  try {
    ...
    console.log(`[OTA] Updating NodeMCU ${device.macAddress} at ${device.ipAddress} (${fileBuffer.length} bytes)...`);

    // ESP8266HTTPUpdateServer expects raw binary POST (not multipart)
    const updateUrl = `http://${device.ipAddress}/update`;
    const response = await fetch(updateUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/octet-stream',
        'Content-Length': fileBuffer.length,
        'x-FileSize': fileBuffer.length
      },
      body: fileBuffer,
    ...
    if (response.ok) {
      res.json({ success: true, message: 'Firmware update completed successfully' });
    } else {
      const errorText = await response.text();
      console.error(`[OTA] Device ${device.ipAddress} rejected update: ${response.status} ${errorText}`);
      res.status(response.status).json({ error: `Device rejected update: ${errorText}` });
    }
  } catch (err) {
    if (req.file && req.file.path && fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
    console.error('[OTA] Error updating NodeMCU firmware:', err.message);
    // Give a more helpful error for common cases
    if (err.name === 'TimeoutError' || err.message.includes('timeout')) {
      res.status(500).json({ error: 'Device did not respond (timeout). Make sure the NodeMCU is online and connected to WiFi.' });
    } else if (err.cause && (err.cause.code === 'ECONNREFUSED' || err.cause.code === 'ENOTFOUND')) {
      res.status(500).json({ error: 'Device is offline or unreachable. Make sure it is powered on and connected to the same network.' });
    } else {
      res.status(500).json({ error: err.message });
    }
  }
  ...
  try {
    ...
    if (manifestVersion || manifestVersionCode) {
        try {
            ...
            // Save release notes locally for the installed version
            if (manifestVersion) {
                try {
                    ...
                    const releaseNotes = {
                        version_name: manifestVersion,
                        version_code: manifestVersionCode || 0,
                        installed_at: new Date().toISOString(),
                        release_notes: manifestData.release_notes || 'No release notes available.'
                    };
                    
                    fs.writeFileSync(
                        path.join(dataDir, 'current_release_notes.json'),
                        JSON.stringify(releaseNotes, null, 2)
                    );
                    console.log(`[System Update] Saved release notes for v${manifestVersion}`);
                } catch (e) {
                    console.warn('[System Update] Could not save release notes:', e.message);
                }
            }
        } catch (e) {
...
// VERSION HISTORY API — List all available versions with release notes
app.get('/api/system/version-history', requireAdmin, async (req, res) => {
    try {
        if (!edgeSync.supabase) {
            return res.status(503).json({ error: 'Cloud sync not configured' });
        }

        const bucketName = 'UPDATE FILE';
        const { data, error } = await edgeSync.supabase.storage
            .from(bucketName)
            .list('system', {
                limit: 100,
        ...
        // Filter for .nxs update files
        const updateFiles = data.filter(f => f.name.endsWith('.nxs'));
        
        // Try to fetch version_history.json for metadata
        let versionHistory = {};
        try {
            const { data: historyData } = await edgeSync.supabase.storage
                .from(bucketName)
                .download('system/version_history.json');
            if (historyData) {
                const text = await historyData.text();
                versionHistory = JSON.parse(text);
            }
        } catch (e) {
            // No history file yet — that's OK
        }

        // Build version list with release notes
        const versions = updateFiles.map(file => {
            // Extract version from filename: RJD-PisoWiFi-v3.12.118-Update.nxs
            const match = file.name.match(/v([\d.]+)-Update\.nxs$/);
            const versionName = match ? match[1] : 'unknown';
            
            // Get metadata from version_history.json if available
            const meta = versionHistory[versionName] || {};
            
            return {
                filename: file.name,
                version_name: versionName,
                version_code: meta.version_code || 0,
                release_notes: meta.release_notes || 'No release notes available.',
                published_at: meta.published_at || file.created_at,
                bucket: bucketName,
...
// ROLLBACK API — Install a specific older version
app.post('/api/system/rollback', requireAdmin, async (req, res) => {
    const { filename, bucket, version_name } = req.body;
    if (!filename) return res.status(400).json({ error: 'Filename is required' });
    ...
    try {
        ...
        console.log(`[Rollback] Downloading ${filename} from bucket ${bucketName}...`);
        ...
        if (downloadError || !downloadData) throw downloadError || new Error('File not found in cloud storage');
        
        // Save to temp file
        const tempPath = path.join(__dirname, 'uploads/backups', `rollback_${Date.now()}.nxs`);
        const dir = path.dirname(tempPath);
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        ...
        console.log(`[Rollback] Downloaded to ${tempPath}. Applying version ${version_name || 'unknown'}...`);
        
        // Apply the update (same logic as regular update)
        await applyUpdate(tempPath, res);
...
// Get release notes for current installed version
app.get('/api/system/current-release-notes', requireAdmin, async (req, res) => {
    try {
        const notesPath = path.join(__dirname, 'data', 'current_release_notes.json');
        if (fs.existsSync(notesPath)) {
            const notes = JSON.parse(fs.readFileSync(notesPath, 'utf8'));
            return res.json(notes);
        }
        
        // Fallback: try to fetch from cloud
        if (edgeSync.supabase) {
            const metaPath = path.join(__dirname, 'metadata.json');
            const meta = JSON.parse(fs.readFileSync(metaPath, 'utf8'));
            
            const { data } = await edgeSync.supabase.storage
                .from('UPDATE FILE')
                .download('system/version_history.json');
            ...
            if (data) {
                ...
                const versionNotes = history[meta.version_name];
                if (versionNotes) {
                    return res.json(versionNotes);
                }
            }
        }
        
        res.json({ release_notes: 'No release notes available for this version.' });
    } catch (err) {
        res.json({ release_notes: 'Unable to load release notes.' });
    }
});
...
// ============================================
// ANTI-TETHERING API ENDPOINTS
// ============================================

app.get('/api/network/anti-tethering/status', requireAdmin, async (req, res) => {
  try {
    const status = await network.getAntiTetheringStatus();
    res.json(status);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.post('/api/network/anti-tethering/install', requireAdmin, async (req, res) => {
  try {
    const result = await network.installIptablesPersistent();
    res.json(result);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.post('/api/network/anti-tethering/apply', requireAdmin, async (req, res) => {
  try {
    const { ttl, interfaces } = req.body;
    if (!ttl || !interfaces || !Array.isArray(interfaces) || interfaces.length === 0) {
      return res.status(400).json({ error: 'TTL value and interfaces array required' });
    }
    const result = await network.applyAntiTethering(Number(ttl), interfaces);
    res.json(result);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.delete('/api/network/anti-tethering', requireAdmin, async (req, res) => {
  try {
    const result = await network.removeAntiTethering();
    res.json(result);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/devices', requireAdmin, async (req, res) => {
  try {
    ...
    // Get registered NodeMCU MACs to exclude from device list
    const nodemcuMacs = new Set(await db.getNodeMCUMacs());
    ...
    const formattedDevices = devices
      .filter(device => {
        if (allowedInterfaces.size > 0 && !allowedInterfaces.has(device.interface)) return false;
        // Exclude registered NodeMCU devices
        if (nodemcuMacs.has(device.mac.toUpperCase())) return false;
        return true;
      })
      .map(device => {
      ...
      return {
        ...
        subscriptionType: device.subscription_type || null,
        subscriptionExpiresAt: device.subscription_expires_at || null
      };
    });
    ...
    // FIX: Verify Online status for stale devices against the live ARP table.
    // last_seen is only refreshed by manual scans, so actively-used devices
    // (e.g. on VLAN interfaces like end0.22) would falsely show Offline.
    // PERFORMANCE: ping probes run in parallel and are capped per request —
    // sequential probing used to stall this endpoint for ~1s per stale device,
    // hanging the UI on large fleets ("Loading devices..." forever).
    try {
      const arpSnapshot = getArpSnapshot();
      const arpMacs = new Set(arpSnapshot.values()); // O(1) MAC lookups
      const nowMs = Date.now();
      const probeCandidates = [];
      for (const device of formattedDevices) {
        if (device.isOnline) continue;
        const mac = (device.mac || '').toUpperCase();
        const ip = (device.ip && device.ip !== 'Unknown') ? device.ip : null;

        // 1. ARP table hit by IP or MAC => device is on the network right now
        let online = false;
        if (ip && arpSnapshot.has(ip)) online = true;
        if (!online && mac && arpMacs.has(mac)) online = true;

        if (online) {
          device.isOnline = true;
          device.lastSeen = nowMs;
          // Persist so subsequent cheap GETs stay correct without re-probing
          db.run('UPDATE wifi_devices SET last_seen = ? WHERE mac = ?', [nowMs, device.mac]).catch(() => {});
          continue;
        }

        // 2. Active session but not in ARP => candidate for a quick ping (throttled)
        if (device.isActive && ip && mac) {
          const lastProbe = _lastDeviceProbe.get(mac) || 0;
          if (nowMs - lastProbe > DEVICE_PROBE_COOLDOWN) {
            probeCandidates.push(device);
          }
        }
      }

      // Probe a capped batch in parallel (each ping is bounded by -W 1).
      // Overflow candidates are picked up on the next poll instead of stalling this one.
      const MAX_PROBES_PER_REQUEST = 25;
      const batch = probeCandidates.slice(0, MAX_PROBES_PER_REQUEST);
      if (batch.length > 0) {
        for (const d of batch) _lastDeviceProbe.set((d.mac || '').toUpperCase(), nowMs);
        const results = await Promise.all(
          batch.map(async d => ({ d, online: await probeDeviceReachable(d.ip) }))
        );
        for (const { d, online } of results) {
          if (online) {
            d.isOnline = true;
            d.lastSeen = nowMs;
            db.run('UPDATE wifi_devices SET last_seen = ? WHERE mac = ?', [nowMs, d.mac]).catch(() => {});
          }
        }
      }
    } catch (e) {
      console.error('[DEVICES] Reachability verification error:', e.message);
    }
...
// Firmware info endpoint — check if update .bin exists on server
app.get('/api/firmware/nodemcu/info', requireAdmin, async (req, res) => {
  try {
    const firmwarePath = path.join(__dirname, 'firmware', 'NodeMCU_ESP8266', 'build', 'esp8266.esp8266.huzzah', 'NodeMCU_ESP8266.ino.bin');
    if (!fs.existsSync(firmwarePath)) {
    ...
    // Read latest firmware version from config
    let latestVersion = '';
    try {
      const row = await db.get('SELECT value FROM config WHERE key = ?', ['latestFirmwareVersion']);
      latestVersion = row?.value || '';
    } catch (_) {}
    res.json({
      available: true,
      size: stats.size,
      ...
      filename: 'NodeMCU_ESP8266.ino.bin',
      latestVersion
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
// Set latest firmware version (admin)
app.post('/api/firmware/nodemcu/version', requireAdmin, async (req, res) => {
  try {
    const { version } = req.body;
    if (typeof version !== 'string' || !version.trim()) {
      return res.status(400).json({ error: 'version is required' });
    }
    await db.run("INSERT OR REPLACE INTO config (key, value) VALUES ('latestFirmwareVersion', ?)", [version.trim()]);
...
// OTA Update from server's compiled firmware (with real-time Socket.IO logs)
app.post('/api/nodemcu/:deviceId/ota-update', requireAdmin, async (req, res) => {
  const { deviceId } = req.params;
  const firmwarePath = path.join(__dirname, 'firmware', 'NodeMCU_ESP8266', 'build', 'esp8266.esp8266.huzzah', 'NodeMCU_ESP8266.ino.bin');

  // Helper to emit OTA log to all connected admin clients
  const emitLog = (level, message) => {
    const ts = new Date().toISOString().replace('T', ' ').slice(0, -1);
    const logLine = `[${ts}] [OTA] [${level}] ${message}`;
    io.emit('ota-log', { deviceId, level, message, timestamp: ts, raw: logLine });
    console.log(logLine);
  };
  ...
  try {
    ...
    if (!fs.existsSync(firmwarePath)) {
      emitLog('ERROR', 'Firmware binary not found on server');
      return res.status(404).json({ error: 'Firmware binary not found on server. Compile the .ino file first.' });
    }

    // 2. Look up device
    const device = await db.getNodeMCUDeviceById(deviceId);
    if (!device) {
      emitLog('ERROR', `Device ${deviceId} not found in database`);
      return res.status(404).json({ error: 'Device not found' });
    }

    if (!device.ipAddress) {
      emitLog('ERROR', `Device ${device.macAddress} has no IP address — it may be offline`);
      return res.status(400).json({ error: 'Device IP address not found. Make sure it has registered recently.' });
    }
    ...
    emitLog('INFO', `═══════════════════════════════════════════════`);
    emitLog('INFO', `OTA Update Started`);
    emitLog('INFO', `Device: ${device.customName || device.hostname || mac}`);
    emitLog('INFO', `MAC:    ${mac}`);
    emitLog('INFO', `IP:     ${ip}`);
    emitLog('INFO', `Firmware size: ${(fileBuffer.length / 1024).toFixed(1)} KB (${fileBuffer.length.toLocaleString()} bytes)`);
    emitLog('INFO', `───────────────────────────────────────────────`);

    // 3. Check if device is reachable (quick ping)
    emitLog('INFO', `Pinging ${ip}...`);
    try {
      await new Promise((resolve, reject) => {
        const pingProc = require('child_process').exec(`ping -c 1 -W 2 ${ip}`, { timeout: 5000 });
        pingProc.on('exit', (code) => {
          if (code === 0) resolve();
          else reject(new Error('Ping failed'));
        });
        pingProc.on('error', reject);
        setTimeout(() => reject(new Error('Ping timeout')), 5000);
      });
      emitLog('SUCCESS', `Device ${ip} is reachable`);
    } catch (pingErr) {
      emitLog('WARN', `Ping to ${ip} failed — device may be offline. Attempting update anyway...`);
    }

    // 4. Send firmware to device (ESP8266HTTPUpdateServer expects multipart form)
    emitLog('INFO', `Sending firmware to http://${ip}/update ...`);
    io.emit('ota-progress', { deviceId, phase: 'uploading', percent: 10 });

    const updateUrl = `http://${ip}/update`;
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 60000); // 60s timeout

    // ESP8266HTTPUpdateServer expects multipart form with field name 'firmware'
    ...
    io.emit('ota-progress', { deviceId, phase: 'uploading', percent: 80 });

    if (response.ok) {
      emitLog('SUCCESS', `Firmware sent successfully!`);
      emitLog('INFO', `Device is rebooting with new firmware...`);
      io.emit('ota-progress', { deviceId, phase: 'rebooting', percent: 95 });

      // 5. Wait a few seconds then verify device comes back
      emitLog('INFO', `Waiting 10s for device to reboot...`);
      await new Promise(r => setTimeout(r, 10000));

      io.emit('ota-progress', { deviceId, phase: 'verifying', percent: 100 });

      // Try pinging again to confirm reboot
      try {
        await new Promise((resolve, reject) => {
          const pingProc = require('child_process').exec(`ping -c 2 -W 3 ${ip}`, { timeout: 10000 });
          pingProc.on('exit', (code) => {
            if (code === 0) resolve();
            else reject(new Error('Ping failed'));
          });
        ...
        emitLog('SUCCESS', `Device ${ip} is back online after reboot!`);
      } catch {
        emitLog('WARN', `Device hasn't responded to ping yet — it may still be booting. Check Hardware page in 30s.`);
      }

      emitLog('INFO', `═══════════════════════════════════════════════`);
      emitLog('SUCCESS', `OTA Update Complete`);
      emitLog('INFO', `═══════════════════════════════════════════════`);
      io.emit('ota-complete', { deviceId, success: true, mac });

      res.json({ success: true, message: 'OTA update completed. Device is rebooting.' });
    } else {
      const errorText = await response.text();
      emitLog('ERROR', `Device rejected update (HTTP ${response.status}): ${errorText}`);
      emitLog('INFO', `═══════════════════════════════════════════════`);
      io.emit('ota-complete', { deviceId, success: false, error: errorText });
      res.status(response.status).json({ error: `Device rejected update: ${errorText}` });
    }
  } catch (err) {
    if (err.name === 'AbortError') {
      emitLog('ERROR', `OTA update timed out (60s) — device not responding`);
    } else {
      emitLog('ERROR', `OTA update failed: ${err.message}`);
    }
    emitLog('INFO', `═══════════════════════════════════════════════`);
    io.emit('ota-complete', { deviceId, success: false, error: err.message });
    res.status(500).json({ error: err.message });
  }
});
  ...
  try {
    ...
    for (const device of scannedDevices) {
      ...
      if (existingDevice) {
        ...
        // ALWAYS re-whitelist devices with active sessions (self-healing: fixes devices that lost iptables rules)
        if (session && device.ip !== 'Unknown') {
          try {
            await network.whitelistMAC(device.mac, device.ip);
          } catch (e) {
            console.log(`[SCAN] Failed to re-whitelist ${device.mac} with IP ${device.ip}: ${e.message}`);
          }
        }
      } else {
        ...
        await db.run(
          'INSERT INTO wifi_devices (id, mac, ip, hostname, interface, ssid, signal, connected_at, last_seen, is_active, download_limit, upload_limit, first_seen_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [id, device.mac, device.ip, device.hostname, device.interface, device.ssid, device.signal, session ? session.connectedAt : now, now, session ? 1 : 0, defaultDl, defaultUl, new Date().toISOString()]
        );
    ...
    // Merge with session data for accurate remaining time (exclude NodeMCU devices)
    const formattedDevices = devices
      .filter(device => !nodemcuMacs.has(device.mac.toUpperCase()))
      .map(device => {
      const deviceMac = device.mac.toUpperCase();
      const session = sessionMap.get(deviceMac);
...
app.post('/api/devices', requireAdmin, async (req, res) => {
  try {
    ...
    await db.run(
      'INSERT INTO wifi_devices (id, mac, ip, hostname, interface, ssid, signal, connected_at, last_seen, is_active, custom_name, first_seen_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, mac.toUpperCase(), ip, hostname || '', iface, ssid || '', signal || 0, now, now, 1, customName || '', new Date().toISOString()]
    );
...
app.put('/api/devices/:id', requireAdmin, async (req, res) => {
  try {
    const { customName, sessionTime, creditPesos, creditMinutes, downloadLimit, uploadLimit, subscriptionType } = req.body;
    const updates = [];
    const values = [];
    ...
    let subscriptionDurationSeconds = null;

    if (subscriptionType !== undefined) {
      if (subscriptionType === 'weekly') {
        subscriptionExpiresAt = Date.now() + 7 * 24 * 60 * 60 * 1000;
        subscriptionDurationSeconds = 7 * 24 * 60 * 60; // 604800s
        ...
        subscriptionDurationSeconds = 30 * 24 * 60 * 60; // 2592000s
        updates.push('subscription_type = ?');
        values.push('monthly');
        updates.push('subscription_expires_at = ?');
        values.push(subscriptionExpiresAt);
        ...
        updates.push('subscription_expires_at = ?');
        values.push(null);
        subscriptionDurationSeconds = 999999999; // ~31 years
      } else {
        // Clear subscription (revert to pay-per-use)
        updates.push('subscription_type = ?');
        values.push(null);
        updates.push('subscription_expires_at = ?');
        values.push(null);
    ...
    // ─── Subscription: Create/Extend Session ────────────────────────────
    if (subscriptionType !== undefined && updatedDevice.mac) {
      const mac = updatedDevice.mac;
      const existingSession = await db.get('SELECT * FROM sessions WHERE mac = ?', [mac]);
      
      // Generate or reuse session token for roaming
      ...
      if (subscriptionDurationSeconds !== null) {
        if (existingSession) {
          ...
          if (subscriptionType === 'none') {
            await db.run(
              'UPDATE sessions SET remaining_seconds = 999999999, rate_expires_at = 0, token = COALESCE(token, ?), is_paused = 0, expired_at = NULL, updated_at = ? WHERE mac = ?',
              [token, new Date().toISOString(), mac]
            );
          } else {
            await db.run(
              'UPDATE sessions SET remaining_seconds = ?, rate_expires_at = ?, token = COALESCE(token, ?), is_paused = 0, expired_at = NULL, updated_at = ? WHERE mac = ?',
              [subscriptionDurationSeconds, subscriptionExpiresAt || 0, token, new Date().toISOString(), mac]
            );
          }
        } else {
          // Create new session
          const now = Date.now();
          const remaining = subscriptionType === 'none' ? 999999999 : subscriptionDurationSeconds;
          await db.run(
            'INSERT INTO sessions (mac, ip, remaining_seconds, total_paid, connected_at, download_limit, upload_limit, token, rate_expires_at, updated_at) VALUES (?, ?, ?, 0, ?, ?, ?, ?, ?, ?)',
            [mac, updatedDevice.ip || '0.0.0.0', remaining, now, updatedDevice.download_limit || 0, updatedDevice.upload_limit || 0, token, subscriptionType === 'none' ? 0 : (subscriptionExpiresAt || 0), new Date().toISOString()]
          );
        }

        // Ensure token is set on session
        if (existingSession && !existingSession.token) {
          await db.run('UPDATE sessions SET token = ? WHERE mac = ?', [token, mac]);
        }

        // Whitelist device immediately
        if (updatedDevice.ip && updatedDevice.ip !== '0.0.0.0') {
          try {
            await network.whitelistMAC(mac, updatedDevice.ip);
          } catch (e) {
            console.warn(`[SUBSCRIPTION] Failed to whitelist ${mac}:`, e.message);
          }
        }

        // Force sync to cloud immediately
        if (edgeSync) {
          const session = await db.get('SELECT * FROM sessions WHERE mac = ?', [mac]);
          if (session) {
            edgeSync.syncDeviceToCloud(mac, session.remaining_seconds || 0, session.total_paid || 0);
          }
        }

        console.log(`[SUBSCRIPTION] ${subscriptionType} subscription set for ${mac} — token=${token.slice(0, 8)}... expires=${subscriptionExpiresAt ? new Date(subscriptionExpiresAt).toISOString() : 'never'}`);
      }
    }
    
    // If session time is being set (non-subscription), also update the active session if device is connected
    if (sessionTime !== undefined && updatedDevice.ip && updatedDevice.mac && subscriptionType === undefined) {
    ...
    if (updatedDevice.ip && updatedDevice.mac && (sessionTime !== undefined || downloadLimit !== undefined || uploadLimit !== undefined || subscriptionType !== undefined)) {
...
// NOTE: This route MUST be defined BEFORE /api/devices/:id to prevent Express capturing "inactive" as :id
...
app.post('/api/devices/:id/connect', requireAdmin, async (req, res) => {
  try {
    ...
    if (existingSession) {
      ...
      await db.run(
        'UPDATE sessions SET remaining_seconds = remaining_seconds + ?, ip = ?, expired_at = NULL WHERE mac = ?',
...
app.post('/api/multiwan/config', requireAdmin, async (req, res) => {
  try {
    ...
    await db.run(
      ...
      [enabled ? 1 : 0, topology || 'single', mode || 'ecmp', pcc_method, JSON.stringify(interfaces)]
    );
    
    // Apply changes via multiwan module (incremental diff, no hotspot disruption)
    await multiwan.apply({ enabled, mode: 'ecmp', topology });
    
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// Multi-WAN health monitoring and routing logic moved to lib/multiwan.js
// The multiwan module handles: ECMP routing, ping-based health checks,
// incremental NAT management, and sandbox iptables chains (RJD_MW_NAT, RJD_MW).
// It never touches initFirewall() rules — hotspot is completely unaffected.

// GET Multi-WAN health status
app.get('/api/multiwan/health', requireAdmin, (req, res) => {
  try {
    res.json({ success: true, health: multiwan.getHealthStatus(), active: multiwan.isActive(), nexthops: multiwan.getActiveNexthops() });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
    ...
    if (newWan.enabled) {
      ...
      if (topology === 'single') {
        ...
        if (enabledWans.length >= 2) {
          ...
          await multiwan.apply({ enabled: true, topology: 'multi' });
        }
      }
    }
...
app.post('/api/multiwan/wans/:id/apply', requireAdmin, async (req, res) => {
  try {
    ...
    if (topology === 'single') {
      ...
      // Re-apply ECMP load balancing via multiwan module
      const enabledWans = await db.all('SELECT * FROM wan_interfaces WHERE enabled = 1');
      if (enabledWans.length >= 2) {
        await multiwan.apply({ enabled: true, topology: 'multi' });
      }
...
async function updateWanTrafficStats() {
  try {
    const wans = await db.all('SELECT * FROM wan_interfaces WHERE enabled = 1');
    // Collect interface names: enabled WANs from DB + default route interface (fallback for unconfigured WAN)
    const ifaceNames = new Set(wans.map(w => w.name));
    try {
      const defaultIface = await network.getDefaultRouteInterface();
      if (defaultIface) ifaceNames.add(defaultIface);
    } catch (e) {}

    for (const name of ifaceNames) {
      try {
        const stats = await network.getWanBytesStats(name);
        const now = Date.now();
        const key = name;
...
updateWanTrafficStats(); // immediate first run so data is available on page load
...
app.get('/api/wireguard/status', requireAdmin, async (req, res) => {
  try {
...
app.post('/api/wireguard/config', requireAdmin, async (req, res) => {
  try {
    let configText = null;

    // Accept either JSON body with configText or file upload
    if (req.body && req.body.configText) {
      configText = req.body.configText;
    } else if (req.files && req.files.config) {
      configText = req.files.config.data.toString('utf8');
    }

    if (!configText) {
      return res.status(400).json({ error: 'No configuration provided. Send configText in JSON body or upload a .conf file.' });
    }

    // Parse and validate
    const parsed = wireguard.parseConfig(configText);

    // Save config
    const metadata = await wireguard.saveConfig(parsed.name, configText, parsed);

    res.json({ success: true, config: metadata });
  } catch (err) {
    console.error('[WireGuard] Config save error:', err);
    res.status(400).json({ error: err.message });
  }
});

app.post('/api/wireguard/toggle', requireAdmin, async (req, res) => {
  try {
    const { enabled } = req.body;
    ...
    let result;
    if (enabled) {
      result = await wireguard.activate();
    } else {
      result = await wireguard.deactivate();
    }

    res.json({ success: true, active: enabled, ...result });
  } catch (err) {
...
app.delete('/api/wireguard/config', requireAdmin, async (req, res) => {
  try {
    await wireguard.deleteConfig();
    res.json({ success: true });
  } catch (err) {
    console.error('[WireGuard] Delete error:', err);
    res.status(500).json({ error: err.message });
  }
});

app.get('/api/wireguard/log', requireAdmin, (req, res) => {
  res.json({ logs: wireguard.getLogs() });
});

app.post('/api/wireguard/log/clear', requireAdmin, (req, res) => {
  wireguard.clearLogs();
  res.json({ success: true });
});
...
async function bootupRestore(isRestricted = false) {
  ...
  // 3.1 Restore Multi-WAN (via multiwan module — handles chains, ECMP, health monitor)
  try {
    console.log('[RJD] Restoring Multi-WAN Configuration...');
    await multiwan.init({ db, network, io });
    // Only clean up legacy PCC rules if Multi-WAN is actually active
    if (multiwan.isActive()) {
      await multiwan.cleanupLegacyRules();
    }
  } catch (e) { console.error('[RJD] Multi-WAN Restore Failed:', e.message); }

  // 3.1.1 Restore WireGuard VPN (if was active before restart)
  try {
    console.log('[RJD] Initializing WireGuard module...');
    await wireguard.init(db, io);
  } catch (e) { console.error('[RJD] WireGuard Init Failed:', e.message); }
  ...
  const coinCallback = (pesos) => {
    ...
    // Log coin detection
    addCoinLogEntry({ type: 'coin_pulse', event: 'Coin Inserted', amount: pesos, slot: 'main', source: 'gpio' });
  };
    ...
    slots.forEach(slot => {
      if (slot.enabled) {
        registerSlotCallback(slot.id, (denomination) => {
          ...
          // Log coin detection from NodeMCU slot
          addCoinLogEntry({ type: 'coin_pulse', event: 'Coin Inserted', amount: denomination, slot: slot.id, slot_name: slot.name || `Slot ${slot.id}`, source: 'nodemcu' });
        });
      }
    });
  }
  ...
  let nodemcuMacs = [];
  try {
    nodemcuMacs = await db.getNodeMCUMacs();
  } catch (e) {
  ...
  try {
    const nodemcuDevicesList = await db.getAllNodeMCUDevices();
    for (const d of nodemcuDevicesList) {
...
const VOUCHER_CODE_REGEX = /^\d{4}$/;

function normalizeVoucherCode(code) {
  return typeof code === 'string' ? code.trim() : '';
}

function isValidVoucherCode(code) {
  return VOUCHER_CODE_REGEX.test(normalizeVoucherCode(code));
}

async function generateUniqueVoucherCode(existingSet = new Set()) {
  const maxAttempts = 100;
  const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'; // A-Z and 0-9
  ...
  throw new Error('Failed to generate unique voucher code - pool may be exhausted');
}
...
app.post('/api/vouchers/generate', requireAdmin, async (req, res) => {
  try {
    ...
    for (let i = 0; i < count; i++) {
      const code = await generateUniqueVoucherCode(generatedCodes);
  ...
  try {
    ...
    if (requestedToken) {
      ...
      if (sessionByToken) {
        if (sessionByToken.mac === mac) {
          await db.run(
            'UPDATE sessions SET remaining_seconds = remaining_seconds + ?, total_paid = total_paid + ?, ip = ?, expired_at = NULL WHERE token = ?',
        ...
        if (existingByMac) {
          ...
          const canonicalToken = hasTime && existingToken ? existingToken : (existingToken || requestedToken);
          await db.run(
            'UPDATE sessions SET remaining_seconds = remaining_seconds + ?, total_paid = total_paid + ?, ip = ?, expired_at = NULL, token = ? WHERE mac = ?',
    ...
    if (!tokenToUse) {
      ...
      tokenToUse = existingSession && existingSession.token ? existingSession.token : crypto.randomBytes(16).toString('hex');
      await db.run(
        'INSERT INTO sessions (mac, ip, remaining_seconds, total_paid, token) VALUES (?, ?, ?, ?, ?) ON CONFLICT(mac) DO UPDATE SET remaining_seconds = remaining_seconds + ?, total_paid = total_paid + ?, ip = ?, expired_at = NULL, token = ?',
...
// ─── Built-in captive-portal DNS responder (port 5354) ───
// blockMAC REDIRECTs blocked devices' DNS here. The responder answers EVERY
// A query with the querying device's own /24 gateway IP — always locally
// reachable on ANY segment (bridge or VLAN), where the portal listens on
// port 80. Zero dependency on system dnsmasq state: this is why the portal
// popup previously only appeared after a pm2 restart (full firewall/DNS
// re-init). The responder works instantly, at runtime, on every subnet.
const CAPTIVE_DNS_PORT = 5354;
let _captiveDnsSocket = null;
let _captiveDnsRetrying = false;

function startCaptivePortalDns() {
  if (_captiveDnsSocket || _captiveDnsRetrying) return;
  try {
    const dgram = require('dgram');
    const dnsServer = dgram.createSocket('udp4');

    dnsServer.on('error', (e) => {
      console.error('[CAPTIVE-DNS] Socket error:', e.message);
      try { dnsServer.close(); } catch (_e) {}
      if (_captiveDnsSocket === dnsServer) _captiveDnsSocket = null;
      if (!_captiveDnsRetrying) {
        _captiveDnsRetrying = true;
        setTimeout(() => { _captiveDnsRetrying = false; startCaptivePortalDns(); }, 5000);
      }
    });

    dnsServer.on('message', (msg, rinfo) => {
      try {
        if (!msg || msg.length < 17) return;
        // Query layout: header(12) + QNAME + QTYPE(2) + QCLASS(2)
        const qtype = msg.readUInt16BE(msg.length - 4);
        if (qtype !== 1) return; // only answer A-record queries

        // Answer with the device's own /24 gateway — always reachable locally
        const parts = String(rinfo.address || '').split('.');
        if (parts.length !== 4) return;
        const octets = [parts[0], parts[1], parts[2], '1'].map(n => parseInt(n, 10) & 0xff);

        const resp = Buffer.alloc(msg.length + 16);
        msg.copy(resp, 0, 0, 12);        // transaction ID
        resp.writeUInt16BE(0x8180, 2);   // flags: QR | RD | RA
        resp.writeUInt16BE(1, 4);        // QDCOUNT
        resp.writeUInt16BE(1, 6);        // ANCOUNT
        msg.copy(resp, 12, 12);          // question section verbatim
        let off = msg.length;
        ...
        dnsServer.send(resp, 0, off, rinfo.port, rinfo.address);
      } catch (_e) { /* a malformed packet must never kill the responder */ }
    });

    dnsServer.bind(CAPTIVE_DNS_PORT, '0.0.0.0', () => {
      console.log(`[CAPTIVE-DNS] Built-in portal DNS responder listening on :${CAPTIVE_DNS_PORT}`);
    });
    if (dnsServer.unref) dnsServer.unref();
    _captiveDnsSocket = dnsServer;
  } catch (e) {
    console.error('[CAPTIVE-DNS] Failed to start:', e.message);
  }
}
...
function startBackgroundTimers() {
  ...
  // Session timer: decrement active sessions every 5s
  // Optimizations: skip when idle, batch expire writes, unref timer
  let _lastActiveCount = -1;
  // Track which MACs the timer has already blocked (to avoid spamming iptables every 5s)
  // Re-block only as a safety net — iptables rules persist until explicitly removed
  const _timerBlockedMacs = new Map(); // mac -> timestamp of last blockMAC call
  const REBLOCK_INTERVAL = 300000; // re-apply blockMAC every 5 min (rules persist indefinitely)
  const _guardBlockedMacs = new Map(); // mac -> timestamp of last FORWARD DROP check (expiredGuardTimer)

  // ─── Startup: batch-mark all stale expired sessions ───
  // After DB corruption/recovery, expired_at may be NULL for all old sessions.
  // Mark them immediately so the timer query doesn't pick them up every 5s.
  (async () => {
    try {
      const now = Date.now();
      await db.run(
        `UPDATE sessions SET expired_at = ? WHERE remaining_seconds <= 0 AND (expired_at IS NULL OR expired_at = 0)`,
        [now]
      );
      console.log('[SESSION] Startup: batch-marked stale expired sessions');
    } catch (e) {}
  })();

  // ─── Startup: repair leaked whitelist rules for 0-time devices ───
  // Older versions left expired_at stamped after a top-up, so expiry blocking
  // never re-fired and devices kept their DNS-bypass (whitelist) rules with
  // 0 time. Detect stale NAT PREROUTING ACCEPT rules for MACs whose session
  // is at 0 and re-block them so the captive portal takes over again.
  setTimeout(async () => {
    try {
      const { stdout } = await execPromise('iptables -t nat -S PREROUTING -n').catch(() => ({ stdout: '' }));
      const whitelisted = new Set();
      for (const line of String(stdout || '').split('\n')) {
        const m = line.match(/-A PREROUTING -m mac --mac-source ([0-9A-Fa-f:]{17}) -j ACCEPT/);
        if (m) whitelisted.add(m[1].toUpperCase());
      }
      for (const mac of whitelisted) {
        // Only touch devices that HAVE a session row at 0 time (the leak signature).
        // Whitelisted MACs without any session row may be rentals/NodeMCU — leave alone.
        const active = await db.get('SELECT mac FROM sessions WHERE mac = ? AND remaining_seconds > 0', [mac]);
        if (active) continue;
        const expired = await db.get('SELECT ip FROM sessions WHERE mac = ? AND remaining_seconds <= 0', [mac]);
        if (!expired) continue;
        console.log(`[SESSION] STARTUP-FIX: Re-blocking ${mac} — whitelist present but 0 time`);
        await network.blockMAC(mac, expired.ip || '');
        io.emit('session-expired', { mac, ip: expired.ip || '' });
      }
    } catch (e) {
      console.error('[SESSION] Startup whitelist repair error:', e.message);
    }
  }, 20000).unref?.();
  
  const sessionTimer = setInterval(async () => {
    try {
      // ─── RATE EXPIRATION SWEEP ───
      // Force-expire sessions whose rate validity window (rate_expires_at) elapsed.
      // Includes PAUSED sessions — pausing freezes the countdown but does NOT
      // extend the rate expiration window (e.g. 5p/2h with 3-day expiration:
      // paused time is forfeited if the client returns after 3 days).
      try {
        const nowMs = Date.now();
        const rateExpired = await db.all(
          `SELECT mac, ip FROM sessions
           WHERE remaining_seconds > 0
           AND rate_expires_at IS NOT NULL AND rate_expires_at > 0 AND rate_expires_at <= ?
           LIMIT 10`,
          [nowMs]
        );
        for (const s of rateExpired) {
          console.log(`[SESSION] RATE-EXPIRED: Blocking ${s.mac} (${s.ip}) — rate expiration window elapsed, remaining time forfeited`);
          await db.run(
            'UPDATE sessions SET remaining_seconds = 0, rate_expires_at = NULL, expired_at = ?, is_paused = 0 WHERE mac = ? AND remaining_seconds > 0',
            [nowMs, s.mac]
          ).catch(() => {});
          await network.blockMAC(s.mac, s.ip || '', true).catch(() => {});
          _timerBlockedMacs.set(s.mac, Date.now());
          io.emit('session-expired', { mac: s.mac, ip: s.ip });
        }
        if (rateExpired.length > 0) _lastActiveCount = -1; // force active-count recount
      } catch (e) {
        console.error('[SESSION] Rate expiration sweep error:', e.message);
      }
      ...
      await db.run(
        `UPDATE sessions SET remaining_seconds = remaining_seconds - 5 
         WHERE remaining_seconds > 0 
         AND (is_paused = 0 OR is_paused IS NULL)
         AND mac NOT IN (
           SELECT mac FROM wifi_devices WHERE subscription_type = 'none'
         )`
      );

      // Check for expired subscriptions (weekly/monthly that passed their expiry date)
      try {
        const expiredSubs = await db.all(
          `SELECT wd.mac, wd.subscription_type FROM wifi_devices wd
           WHERE wd.subscription_type IN ('weekly', 'monthly')
           AND wd.subscription_expires_at IS NOT NULL
           AND wd.subscription_expires_at > 0
           AND wd.subscription_expires_at < ?`,
          [Date.now()]
        );
        for (const sub of expiredSubs) {
          // Subscription expired — clear it and let normal expiry handle the session
          await db.run(
            "UPDATE wifi_devices SET subscription_type = NULL, subscription_expires_at = NULL WHERE mac = ?",
            [sub.mac]
          );
          console.log(`[SUBSCRIPTION] ${sub.subscription_type} expired for ${sub.mac} — cleared`);
        }
      } catch (e) {
        console.warn('[SUBSCRIPTION] Expiry check error:', e.message);
      }

      // CRITICAL: Find sessions with 0 time that have NOT yet been marked expired
      // LIMIT 10 to prevent mass iptables spam after DB recovery
      // NOT EXISTS subquery ensures we skip MACs that just got a new session
      const expired = await db.all(
        `SELECT DISTINCT s.mac, s.ip FROM sessions s 
         WHERE s.remaining_seconds <= 0 
         AND (s.expired_at IS NULL OR s.expired_at = 0)
         AND NOT EXISTS (SELECT 1 FROM sessions s2 WHERE s2.mac = s.mac AND s2.remaining_seconds > 0)
         LIMIT 10`
      );
      ...
      for (const s of expired) {
        // Skip if we already blocked this MAC recently (within REBLOCK_INTERVAL)
        const lastBlock = _timerBlockedMacs.get(s.mac);
        if (lastBlock && (Date.now() - lastBlock) < REBLOCK_INTERVAL) {
          // Still mark as expired in DB so the query doesn't pick it up again
          await db.run(
            'UPDATE sessions SET expired_at = ? WHERE mac = ? AND remaining_seconds <= 0 AND (expired_at IS NULL OR expired_at = 0)',
            [Date.now(), s.mac]
          ).catch(() => {});
          continue;
        }

        // CRITICAL: Mark as expired FIRST — before blockMAC — so the query
        // never picks up this session again even if blockMAC is slow or fails
        await db.run(
          'UPDATE sessions SET expired_at = ? WHERE mac = ? AND remaining_seconds <= 0 AND (expired_at IS NULL OR expired_at = 0)',
          [Date.now(), s.mac]
        ).catch(() => {});

        // PROACTIVE BLOCK: Call blockMAC to ensure DNS redirect rules are in place
        // force=true — a stale cooldown must never skip the exact-expiry block
        console.log(`[SESSION] EXPIRED: Blocking ${s.mac} (${s.ip}) - time reached 0`);
        await network.blockMAC(s.mac, s.ip, true);
        _timerBlockedMacs.set(s.mac, Date.now());

        // Notify connected portal clients to force captive portal popup
        io.emit('session-expired', { mac: s.mac, ip: s.ip });
      }
    } catch (e) { console.error(e); }
  }, 5000); // Increased from 2s to 5s — 1s accuracy not needed for session countdown
  if (sessionTimer.unref) sessionTimer.unref();

  // ─── GUARD: Fully block ALL internet for 0-time devices ───
  // The sessionTimer above calls blockMAC which only does DNS hijack.
  // Apps like Facebook Messenger use cached/hardcoded IPs, bypassing DNS.
  // This guard adds FORWARD chain REJECT rules to block ALL traffic for expired MACs.
  // IMPORTANT: REJECT with tcp-reset (NOT silent DROP). A silent DROP makes OS
  // captive-portal probes time out, which Android/iOS/Windows interpret as plain
  // "no internet" — the portal popup NEVER appears. An instant TCP RST forces the
  // OS to re-evaluate the connection, re-query DNS (hijacked to portal), and the
  ...
  const expiredGuardTimer = setInterval(async () => {
    try {
      // SELF-HEALING RECONCILIATION: verify the actual firewall state for ALL
      // 0-time devices — regardless of the expired_at marker. A stale marker,
      // a skipped cooldown, or a leaked whitelist rule must never leave a
      // 0-time device with internet access.
      const now = Date.now();
      const expiredSessions = await db.all(
        `SELECT DISTINCT mac, ip, expired_at FROM sessions
         WHERE remaining_seconds <= 0
         AND mac IS NOT NULL AND mac != ''
         AND NOT EXISTS (SELECT 1 FROM sessions s2 WHERE s2.mac = sessions.mac AND s2.remaining_seconds > 0)
         LIMIT 30`
      );

      // ── BATCH: dump iptables ONCE for all guard checks (Phase 1 + Phase 2) ──
      // With 200+ VLANs, per-MAC `iptables -C` spawns 2+ processes each.
      // Instead, dump all rules once and check in memory.
      let natRulesText = '';
      let fwdRulesText = '';
      try {
        const [natRes, fwdRes] = await Promise.all([
          execPromise('iptables-save -t nat 2>/dev/null || echo ""'),
          execPromise('iptables-save -t filter 2>/dev/null || echo ""')
        ]);
        natRulesText = natRes.stdout || '';
        fwdRulesText = fwdRes.stdout || '';
      } catch (_e) {}
      ...
      const forwardRejectMacsGuard = new Set();
      if (fwdRulesText) {
        const frRegex = /-A FORWARD -m mac --mac-source ([0-9A-Fa-f:]+) -j REJECT/g;
        let m;
        while ((m = frRegex.exec(fwdRulesText)) !== null) forwardRejectMacsGuard.add(m[1].toUpperCase());
      }

      for (const s of expiredSessions) {
        // Skip if we already verified this MAC recently
        const lastGuard = _guardBlockedMacs.get(s.mac) || 0;
        if ((now - lastGuard) < EXPIRED_GUARD_RECHECK_INTERVAL) continue;
        _guardBlockedMacs.set(s.mac, now);

        // Rental devices keep internet by design — never guard them
        try { if (await isRentalDeviceActive(s.mac)) continue; } catch (_e) {}

        // 1. Stale whitelist (paid bypass) left on a 0-time device? This is the
        // "free internet" leak — force-block removes it and re-applies DNS hijack.
        // BATCH: use in-memory Set instead of per-MAC iptables -C
        const macUpper = s.mac.toUpperCase();
        const whitelistLeak = whitelistedMacsGuard.has(macUpper);
        if (whitelistLeak) {
          console.log(`[SESSION] GUARD: ${s.mac} has stale whitelist with 0 time — force-blocking`);
          await network.blockMAC(s.mac, s.ip || '', true);
        }

        // 2. Ensure FORWARD REJECT rule exists (blocks ALL traffic incl. cached IPs)
        // REJECT with tcp-reset (NOT silent DROP) so OS probes fail fast and
        // re-trigger captive-portal detection (popup).
        // BATCH: use in-memory Set instead of per-MAC iptables -C
        if (!forwardRejectMacsGuard.has(macUpper)) {
          // Rule doesn't exist — insert it (and remove the old silent DROP if present)
          await execPromise(`iptables -D FORWARD -m mac --mac-source ${s.mac} -j DROP`).catch(() => {});
          await execPromise(`iptables -I FORWARD -m mac --mac-source ${s.mac} -j REJECT --reject-with tcp-reset`).catch(() => {});
        }

        // 3. Keep the expired_at marker consistent (UI/other detectors)
        if (!s.expired_at) {
          await db.run(
            'UPDATE sessions SET expired_at = ? WHERE mac = ? AND remaining_seconds <= 0 AND (expired_at IS NULL OR expired_at = 0)',
            [now, s.mac]
        ...
        // 4. Rate-limited Socket.IO notification to force portal popup
        const lastNotify = _expiredGuardNotified.get(s.mac) || 0;
        if (now - lastNotify > EXPIRED_GUARD_NOTIFY_INTERVAL) {
          io.emit('session-expired', { mac: s.mac, ip: s.ip });
          _expiredGuardNotified.set(s.mac, now);
        }
      }

      // ─── PHASE 2: ARP SWEEP — ANY device online with no time gets blocked ───
      // Covers zero-time devices AND devices that never had a session row.
      // Every LAN entry is verified against the firewall; paid sessions,
      // rentals and NodeMCU coinslots are exempt.
      try {
        const wanDevs = new Set();
        try {
          const defWan = await network.getDefaultRouteInterface?.();
          if (defWan) wanDevs.add(defWan);
          const wans = await db.all('SELECT name FROM wan_interfaces WHERE enabled = 1').catch(() => []);
          for (const w of wans) if (w && w.name) wanDevs.add(String(w.name).trim());
        } catch (_e) {}

        // ── BATCH: reuse natRulesText/fwdRulesText already parsed above ──
        // (No duplicate iptables-save call — Phase 1 dump covers Phase 2 too)
        // Parse which interfaces already have portal redirect rules
        const portalRuleDevs = new Set();
        if (natRulesText) {
          const portalRegex = /-A PREROUTING -i (\S+) -p tcp -m tcp --dport 80 -j REDIRECT --to-ports 80/g;
          let m;
          while ((m = portalRegex.exec(natRulesText)) !== null) portalRuleDevs.add(m[1]);
        }

        // Collect interfaces that need portal rules (batch add at end)
        const devsNeedingPortalRules = [];

        const arpEntries = getArpSnapshotByDevice();
        for (const [ip, entry] of arpEntries) {
          const mac = entry.mac;
          if (!mac) continue;
          if (entry.dev && wanDevs.has(entry.dev)) continue; // NEVER touch WAN-side devices

          // Self-heal: make sure this segment has the portal redirect rules.
          // (VLANs created after boot may have missed them until a pm2 restart.)
          if (entry.dev && !portalRuleDevs.has(entry.dev)) {
            portalRuleDevs.add(entry.dev); // mark as seen
            devsNeedingPortalRules.push(entry.dev);
          }

          const key = `arp:${mac}`;
          const lastCheck = _guardBlockedMacs.get(key) || 0;
          if ((now - lastCheck) < EXPIRED_GUARD_RECHECK_INTERVAL) continue;
          _guardBlockedMacs.set(key, now);

          // Paid + running session → leave alone
          ...
          // Rental devices keep internet by design
          let rental = false;
          try { rental = await isRentalDeviceActive(mac); } catch (_e) {}
          if (rental) continue;

          // NodeMCU coinslots must always reach the server
          const coinslot = await db.get(
            'SELECT id FROM nodemcu_devices WHERE mac_address = ?',
            [mac]
          ).catch(() => null);
          if (coinslot) continue;

          // Zero-time or never-paid device online → enforce blocked posture
          // Use in-memory check instead of iptables -C
          const macUpper = mac.toUpperCase();
          const whitelistLeak = whitelistedMacsGuard.has(macUpper);
          if (whitelistLeak) {
            console.log(`[SESSION] GUARD-ARP: ${mac} (${ip}) online with no time — force-blocking`);
            await network.blockMAC(mac, ip, true);
          }
          if (!forwardRejectMacsGuard.has(macUpper)) {
            await execPromise(`iptables -D FORWARD -m mac --mac-source ${mac} -j DROP`).catch(() => {});
            await execPromise(`iptables -I FORWARD -m mac --mac-source ${mac} -j REJECT --reject-with tcp-reset`).catch(() => {});
          }
          const lastNotify = _expiredGuardNotified.get(mac) || 0;
          if (now - lastNotify > EXPIRED_GUARD_NOTIFY_INTERVAL) {
            io.emit('session-expired', { mac, ip });
            _expiredGuardNotified.set(mac, now);
          }
        }

        // Batch-add portal redirect rules for interfaces that were missing them
        for (const dev of devsNeedingPortalRules) {
          console.log(`[SESSION] GUARD-ARP: adding portal redirect rules to ${dev}`);
          await execPromise(`iptables -t nat -A PREROUTING -i ${dev} -p tcp --dport 80 -j REDIRECT --to-ports 80`).catch(() => {});
          await execPromise(`iptables -t nat -A PREROUTING -i ${dev} -p udp --dport 53 -j REDIRECT --to-ports 5354`).catch(() => {});
          await execPromise(`iptables -t nat -A PREROUTING -i ${dev} -p tcp --dport 53 -j REDIRECT --to-ports 5354`).catch(() => {});
          await execPromise(`iptables -A FORWARD -i ${dev} -p tcp --dport 443 -j REJECT --reject-with tcp-reset`).catch(() => {});
        }
      } catch (e) {
        console.error('[SESSION] GUARD-ARP sweep error:', e.message);
      }

      // Cleanup: remove old entries to prevent unbounded growth
      if (_guardBlockedMacs.size > 200 || _expiredGuardNotified.size > 200) {
        for (const [mac, ts] of _guardBlockedMacs) {
          if (now - ts > EXPIRED_GUARD_RECHECK_INTERVAL * 2) _guardBlockedMacs.delete(mac);
        }
        for (const [mac, ts] of _expiredGuardNotified) {
          if (now - ts > EXPIRED_GUARD_NOTIFY_INTERVAL * 2) _expiredGuardNotified.delete(mac);
        }
      }
    } catch (_e) {
  ...
  if (expiredGuardTimer.unref) expiredGuardTimer.unref();

  // ─── Active Session Self-Healer: re-whitelist devices with valid time that lost iptables rules ───
  const activeSessionHealerTimer = setInterval(async () => {
    try {
      // Get all active sessions with valid time
      const activeSessions = await db.all(
        'SELECT mac, ip FROM sessions WHERE remaining_seconds > 0 AND (is_paused = 0 OR is_paused IS NULL)'
      );
      
      if (activeSessions.length === 0) return;

      // BATCH: dump iptables ONCE instead of per-session iptables -C
      let natRulesText = '';
      try {
        const res = await execPromise('iptables-save -t nat 2>/dev/null || echo ""');
        natRulesText = res.stdout || '';
      } catch (_e) {}

      // Parse which MACs already have whitelist ACCEPT rules
      const whitelistedMacs = new Set();
      if (natRulesText) {
        const wlRegex = /-A PREROUTING -m mac --mac-source ([0-9A-Fa-f:]+) -j ACCEPT/g;
        let m;
        while ((m = wlRegex.exec(natRulesText)) !== null) whitelistedMacs.add(m[1].toUpperCase());
      }

      for (const session of activeSessions) {
        if (!session.mac || !session.ip || session.ip === 'Unknown') continue;
        
        // In-memory check instead of per-session iptables -C
        if (!whitelistedMacs.has(session.mac.toUpperCase())) {
          console.log(`[SESSION] HEALER: ${session.mac} has valid time but missing whitelist — re-whitelisting`);
          try {
            await network.whitelistMAC(session.mac, session.ip);
          } catch (e) {
            console.error(`[SESSION] HEALER: Failed to re-whitelist ${session.mac}:`, e.message);
          }
        }
      }
    } catch (e) {
      console.error('[SESSION] HEALER error:', e.message);
    }
  }, 30000); // every 30 seconds
  if (activeSessionHealerTimer.unref) activeSessionHealerTimer.unref();
  ...
  const tcCleanupTimer = setInterval(async () => {
    try {
      ...
      // ── BATCH: only call removeSpeedLimit for IPs that actually have QoS marks ──
      // With 200+ inactive sessions, calling removeSpeedLimit per session spawns
      // 20+ processes each (4000+ total). Instead, check mangle table once.
      ...
      if (inactiveSessions.length > 0) {
        let mangleMarkedIPs = new Set();
        try {
          const { stdout: mangleText } = await execPromise('iptables-save -t mangle 2>/dev/null || echo ""');
          if (mangleText) {
            const markRegex = /-A RJD_QOS_MARK -[sd] (\d+\.\d+\.\d+\.\d+)/g;
            let mm;
            while ((mm = markRegex.exec(mangleText)) !== null) mangleMarkedIPs.add(mm[1]);
          }
        } catch (_e) {}

        for (const session of inactiveSessions) {
          if (mangleMarkedIPs.has(session.ip)) {
            await network.removeSpeedLimit(session.mac, session.ip);
          }
        }
      }

      // Skip nft/tc orphan cleanup when there are no inactive sessions
      ...
      // ─── TC ORPHAN CLEANUP (every 5 min only — expensive with 200+ interfaces) ───
      // Most cleanup is handled by removeSpeedLimit above. This scan is a safety net.
      if (!global._lastTcOrphanScan || (Date.now() - global._lastTcOrphanScan) < 300000) {
        if (!global._lastTcOrphanScan) global._lastTcOrphanScan = Date.now();
        // Skip — not time yet
      } else {
        global._lastTcOrphanScan = Date.now();
        const { stdout: interfacesOutput } = await execPromise(`ip link show | grep -E "eth|en|wlan|br|vlan|ifb|end" | awk '{print $2}' | sed 's/:$//'`).catch(() => ({ stdout: '' }));
        const interfaces = interfacesOutput.trim().split('\n').filter(i => i);
        for (const iface of interfaces) {
          try {
            // Single tc filter dump per interface (was 3 separate calls — 66% reduction)
            const { stdout: allFilters } = await execPromise(`tc filter show dev ${iface} 2>/dev/null || echo ""`).catch(() => ({ stdout: '' }));
            if (!allFilters || !allFilters.trim()) continue;

            // Parse all IPs from dst matches (download egress)
            const dstIPs = allFilters.match(/match ip dst (\d+\.\d+\.\d+\.\d+)/g) || [];
            for (const m of dstIPs) {
              const ip = m.match(/\d+\.\d+\.\d+\.\d+/)[0];
              if (!activeIPs.has(ip)) {
            ...
            // Parse all IPs from src matches (upload egress)
            const srcIPs = allFilters.match(/match ip src (\d+\.\d+\.\d+\.\d+)/g) || [];
            for (const m of srcIPs) {
              const ip = m.match(/\d+\.\d+\.\d+\.\d+/)[0];
              if (!activeIPs.has(ip)) {
                const parts = ip.split('.');
                const cId = (parseInt(parts[2]) << 8 | parseInt(parts[3])) & 0x7FFF;
            ...
            // Parse ffff: ingress filters (legacy)
            const ingressIPs = allFilters.match(/parent ffff:.*?(\d+\.\d+\.\d+\.\d+)/g) || [];
            for (const m of ingressIPs) {
              const ip = m.match(/\d+\.\d+\.\d+\.\d+/)[0];
              if (!activeIPs.has(ip)) {
  ...
  }, 60000); // 60s for mangle cleanup; TC orphan scan throttled to every 5 min internally
...
(async () => {
  try {
    await db.init();
    // Load persistent coin detection log from DB
    await loadCoinLogFromDb();
  } catch (e) {
  ...
  server.listen(80, '0.0.0.0', async () => {
    ...
    console.log(`[SECURITY] Hardware API Key: ${HARDWARE_API_KEY} (add to NodeMCU firmware as X-Hardware-Key header)`);
...
app.post('/api/free-internet/claim', async (req, res) => {
  try {
    ...
    if (!existingDevice) {
      await db.run(
        'INSERT INTO wifi_devices (id, mac, ip, hostname, interface, ssid, signal, connected_at, last_seen, is_active, custom_name, credit_pesos, credit_minutes, first_seen_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [mac, mac, clientIp, 'FreeInternet', 'wlan0', 'FreeInternet', 0, Date.now(), Date.now(), 1, '', 0, 0, new Date().toISOString()]
      );
    }
    ...
    if (existingSession) {
      ...
      await db.run(
        'UPDATE sessions SET remaining_seconds = remaining_seconds + ?, total_paid = total_paid + 0, expired_at = NULL WHERE mac = ?',
        [seconds, mac]
      );
    } else {
      // Create new session
...
app.post('/api/phone-rental/devices/:deviceId/wallpaper', requireAdmin, async (req, res) => {
  try {
    const { deviceId } = req.params;
...
app.delete('/api/phone-rental/devices/:deviceId/wallpaper', requireAdmin, async (req, res) => {
  try {
    const { deviceId } = req.params;
