...
export const apiClient = {
  ...
  async getAntiTetheringStatus(): Promise<{ installed: boolean; enabled: boolean; ttl_value: number; interfaces: string[] }> {
    const res = await fetch(`${API_BASE}/network/anti-tethering/status`, { headers: getHeaders() });
    return handleResponse(res);
  },

  async installIptablesPersistent(): Promise<{ success: boolean }> {
    const res = await fetch(`${API_BASE}/network/anti-tethering/install`, {
      method: 'POST',
      headers: getHeaders()
    });
    return handleResponse(res);
  ...
  async applyAntiTethering(ttl: number, interfaces: string[]): Promise<{ success: boolean }> {
    const res = await fetch(`${API_BASE}/network/anti-tethering/apply`, {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify({ ttl, interfaces })
    });
    return handleResponse(res);
  },

  async removeAntiTethering(): Promise<{ success: boolean }> {
    const res = await fetch(`${API_BASE}/network/anti-tethering`, {
      method: 'DELETE',
      headers: getHeaders()
    });
    return handleResponse(res);
  ...
  async getFirmwareInfo(): Promise<any> {
    const res = await fetch(`${API_BASE}/firmware/nodemcu/info`, { headers: getHeaders() });
    return handleResponse(res);
  },

  async otaUpdateNodeMCU(deviceId: string): Promise<any> {
    const res = await fetch(`${API_BASE}/nodemcu/${deviceId}/ota-update`, {
      method: 'POST',
      headers: getHeaders()
    });
    return handleResponse(res);
