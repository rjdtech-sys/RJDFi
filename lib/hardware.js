...
async function getUniqueHardwareId() {
  if (cachedHardwareId && (cachedHardwareId.startsWith('CPU-') || cachedHardwareId.startsWith('MAC-') || cachedHardwareId.startsWith('WIN-'))) {
    return cachedHardwareId;
  }
  const serial = getHardwareSerial();
  if (serial) {
    cachedHardwareId = `CPU-${serial}`;
    return cachedHardwareId;
  }
  ...
  try {
    ...
    if (process.platform === 'win32') {
      ...
      if (firstMac) {
        cachedHardwareId = `WIN-${firstMac.replace(/-/g, '')}`;
        return cachedHardwareId;
      }
    }
    ...
    if (macMatch && macMatch[1]) {
      cachedHardwareId = `MAC-${macMatch[1].replace(/:/g, '')}`;
      return cachedHardwareId;
    }
  } catch (error) {
