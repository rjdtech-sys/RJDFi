...
class EdgeSync {
  ...
  async registerOrFetchMachine() {
    ...
    try {
      // 1. First check licenses table in Supabase for hardware_id to get vendor_id or created_by
      try {
        ...
        if (licData) {
          const effectiveVendorId = licData.vendor_id || licData.created_by;
          if (effectiveVendorId) {
            this.vendorId = effectiveVendorId;
            this.saveLocalVendorId(effectiveVendorId);
          }
      ...
      // 2. Check if machine exists in vendors table
      ...
      if (data) {
        ...
        // Register new machine with valid vendor_id
        if (!this.vendorId) {
          try {
        ...
        if (!newMachinePayload.vendor_id) {
          console.warn('[EdgeSync] Skipping vendors insert — vendor_id not set yet. Status and revenues will sync directly via hardware_id.');
          return;
        }
