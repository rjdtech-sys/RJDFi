...
class EdgeSync {
  ...
  async syncMachineStatus(status) {
    ...
    try {
      ...
      // Also update hardware_licenses table in Supabase so Cloud License Dashboard matches actual machine revenues
      if (this.hardwareId) {
        try {
          await this.supabase
          ...
          // Ignore if hardware_licenses table schema is slightly different
        }
      }
