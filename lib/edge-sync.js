...
class EdgeSync {
    ...
    try {
      ...
      if (data) {
        ...
        if (data.vendor_id) {
          ...
          // Fill in missing vendor_id in existing vendor record
          await this.supabase
            .from('vendors')
            .update({ vendor_id: vendorId, status: 'online' })
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(sessions) && sessions.length > 0) {
          let sDaily = 0, sWeekly = 0, sMonthly = 0, sYearly = 0, sTotal = 0;
          for (const s of sessions) {
            ...
            sTotal += amt;
            if (ts >= todayStart) sDaily += amt;
            if (ts >= weekStart) sWeekly += amt;
            if (ts >= monthStart) sMonthly += amt;
            if (ts >= yearStart) sYearly += amt;
          }
          if (sTotal > total) {
            daily = sDaily; weekly = sWeekly; monthly = sMonthly; yearly = sYearly; total = sTotal;
          }
        }
      } catch (e) {}
  ...
  async syncMachineStatus(status = 'online') {
    ...
    try {
      ...
      // 2. Update licenses table in Supabase (Cloud License Platform Dashboard shown in Image 1)
      if (this.hardwareId) {
        try {
