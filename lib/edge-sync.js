...
class EdgeSync {
  ...
  async registerOrFetchMachine() {
    ...
    try {
      // 1. First check licenses table in Supabase for hardware_id to get vendor_id
      try {
        const { data: licData } = await this.supabase
          .from('licenses')
          .select('vendor_id, license_key')
          .eq('hardware_id', this.hardwareId)
          .maybeSingle();

        if (licData && licData.vendor_id) {
          this.vendorId = licData.vendor_id;
          this.saveLocalVendorId(licData.vendor_id);
        }
      } catch (e) {}
      ...
      if (data) {
        ...
        // Register new machine (Online Status)
        ...
        if (this.vendorId) {
            ...
            try {
                // Fetch existing vendor_id from cloud if available
                ...
                if (vRow && vRow.vendor_id) {
                    newMachinePayload.vendor_id = vRow.vendor_id;
