...
// Roaming session push cadence: one batched upsert of only MEANINGFULLY CHANGED sessions.
// 60s bounds worst-case cloud staleness of remaining_seconds to ~1 minute while costing
// a single tiny HTTPS request per minute (<0.5% CPU).
const ROAMING_SESSION_PUSH_INTERVAL = 60000;
...
class EdgeSync {
  constructor() {
    ...
    this.wifiDevicesSupported = true;
    this.machineCommandsSupported = true;
  ...
  async syncCloudWifiDevicesToLocal() {
    if (!this.supabase || !this.vendorId || !this.wifiDevicesSupported) return;
    ...
    try {
      ...
      if (error) {
        if (error.message && (error.message.includes('wifi_devices') || error.message.includes('schema cache') || error.code === 'PGRST205' || error.code === '42P01')) {
          this.wifiDevicesSupported = false;
          return;
        }
        throw error;
      }
      ...
      if (e && e.message && (e.message.includes('wifi_devices') || e.message.includes('schema cache'))) {
        this.wifiDevicesSupported = false;
        return;
      }
  ...
  async saveLocalVendorId(id) {
    if (!id) return;
    try {
      this.vendorId = id;
      await db.run('INSERT OR REPLACE INTO config (key, value) VALUES (?, ?)', ['cloud_vendor_id', id]);
    } catch (e) {}
  }

  async loadLocalVendorId() {
    if (this.vendorId) return this.vendorId;
    try {
  ...
  /**
   * Resolve valid vendor_id and update null vendor_id on old cloud licenses
   */
  async findValidVendorId() {
    if (this.vendorId) return this.vendorId;

    const local = await this.loadLocalVendorId();
    if (local) return local;

    if (!this.supabase) return null;

    // 1. Query licenses table for this hardware_id
    try {
      const { data: licData } = await this.supabase
        .from('licenses')
        .select('vendor_id, created_by, license_key')
        .eq('hardware_id', this.hardwareId)
        .maybeSingle();

      if (licData) {
        const effective = licData.vendor_id || licData.created_by;
        if (effective) {
          await this.saveLocalVendorId(effective);

          // If vendor_id was null/empty in cloud licenses table, auto-update it with account UUID!
          if (!licData.vendor_id) {
            try {
              await this.supabase
                .from('licenses')
                .update({ vendor_id: effective })
                .eq('hardware_id', this.hardwareId);
              console.log(`[EdgeSync] Updated old cloud license vendor_id to: ${effective}`);
            } catch (_) {}
          }

          return effective;
        }
        if (licData.license_key) {
          this.centralizedKey = licData.license_key;
        }
      }
    } catch (_) {}

    // 2. Query vendors table for any existing vendor_id
    try {
      ...
      if (vRow && vRow.vendor_id) {
        await this.saveLocalVendorId(vRow.vendor_id);
        return vRow.vendor_id;
      }
    } catch (_) {}

    // 3. Query user_roles table for any valid user account UUID
    try {
      const { data: urRow } = await this.supabase
        .from('user_roles')
        .select('user_id')
        .limit(1)
        .maybeSingle();

      if (urRow && urRow.user_id) {
        await this.saveLocalVendorId(urRow.user_id);
        return urRow.user_id;
      }
    } catch (_) {}
  ...
  async registerOrFetchMachine() {
    ...
    try {
      const vendorId = await this.findValidVendorId();

      // Check if machine exists in vendors table
      const { data, error } = await this.supabase
        .from('vendors')
        .select('id, hardware_id, vendor_id')
        .eq('hardware_id', this.hardwareId)
        .maybeSingle(); 
      
      if (error) {
        console.error('[EdgeSync] Supabase select error:', error.message || error);
      }
      ...
      if (data) {
        ...
        const updateObj = { 
          status: 'online', 
          is_licensed: true, 
          license_key: this.centralizedKey || 'RJD-PISO-63C4DE5B-93AC2ADA',
          location: 'Main Branch'
        };

        if (data.vendor_id) {
          this.vendorId = data.vendor_id;
          await this.saveLocalVendorId(data.vendor_id);
        } else if (vendorId) {
          updateObj.vendor_id = vendorId;
        }
        ...
        const newMachinePayload = {
            ...
            machine_name: `Orange Pi One (${this.hardwareId.substring(0, 8)})`,
            status: 'online',
            is_licensed: true,
            location: 'Main Branch',
            license_key: this.centralizedKey || 'RJD-PISO-63C4DE5B-93AC2ADA',
        ...
        if (vendorId) {
          newMachinePayload.vendor_id = vendorId;
        }

        console.log('[EdgeSync] Registering machine with payload:', JSON.stringify(newMachinePayload, null, 2));
        ...
        if (insertError) {
          console.error('[EdgeSync] Supabase insert error:', insertError.message || insertError);
        } else if (newData) {
          this.machineId = newData.id;
          console.log(`[EdgeSync] New machine registered online: ${this.machineId}`);
        }
      }
    } catch (err) {
  ...
  startStatusSync() {
    ...
    // Dedicated fast roaming push: keeps cloud remaining_seconds fresh (~60s worst case)
    // so another board can restore accurate time when a client roams. Single batched
    // request per cycle — change filter keeps payload (and CPU) minimal.
    if (!this.roamingPushInterval) {
      this.roamingPushInterval = setInterval(() => {
        this.syncClientsToCloud();
      }, ROAMING_SESSION_PUSH_INTERVAL);
      if (this.roamingPushInterval.unref) this.roamingPushInterval.unref();
    }
  ...
  stopStatusSync() {
    ...
    if (this.roamingPushInterval) {
      clearInterval(this.roamingPushInterval);
      this.roamingPushInterval = null;
    }
  }
  ...
  /**
   * Calculate local machine revenues (Daily, Weekly, Monthly, Yearly, Lifetime Total)
   */
  async calculateRevenues() {
    try {
      const now = new Date();
      ...
      // 1. Check sales table (Primary)
      try {
        const salesRows = await db.all('SELECT amount, created_at, timestamp FROM sales WHERE amount > 0');
        if (Array.isArray(salesRows) && salesRows.length > 0) {
          for (const s of salesRows) {
            const amt = parseFloat(s.amount) || 0;
            const rawTs = s.timestamp || s.created_at;
            const ts = typeof rawTs === 'number' ? rawTs : new Date(rawTs).getTime();
            if (isNaN(ts)) continue;

            total += amt;
            if (ts >= todayStart) daily += amt;
            if (ts >= weekStart) weekly += amt;
            if (ts >= monthStart) monthly += amt;
            if (ts >= yearStart) yearly += amt;
          }
          if (total > 0) {
      ...
      // 2. Check coin_log table
      try {
        const logs = await db.all('SELECT timestamp, amount FROM coin_log WHERE amount > 0');
      ...
      // 3. Fallback: sessions table total_paid
      try {
        const sessions = await db.all('SELECT connected_at, total_paid FROM sessions WHERE total_paid > 0');
        if (Array.isArray(sessions) && sessions.length > 0) {
          for (const s of sessions) {
            const amt = parseFloat(s.total_paid) || 0;
            const ts = typeof s.connected_at === 'number' ? s.connected_at : new Date(s.connected_at).getTime();
            if (isNaN(ts)) continue;
   ...
   */
  async syncMachineStatus(status = 'online') {
    if (!this.supabase) return false;

    if (!this.machineId && this.hardwareId) {
      await this.registerOrFetchMachine();
    }

    try {
      const metrics = await this.getMetrics();
      const revenues = await this.calculateRevenues();
      ...
      const updatePayload = {
        ...
        daily_revenue: revenues.daily,
        weekly_revenue: revenues.weekly,
        monthly_revenue: revenues.monthly,
        yearly_revenue: revenues.yearly
      };

      if (this.vendorId) {
        updatePayload.vendor_id = this.vendorId;
      }

      // 1. Update vendors table in Supabase by hardware_id and machineId
      try {
        await this.supabase
          .from('vendors')
      ...
      // 2. Update licenses table in Supabase
      if (this.hardwareId) {
        try {
          const licUpdate = {
            daily_revenue: revenues.daily,
            ...
            yearly_revenue: revenues.yearly,
            total_revenue: revenues.total,
            daily: revenues.daily,
            weekly: revenues.weekly,
            monthly: revenues.monthly,
            yearly: revenues.yearly,
            total: revenues.total,
            is_active: true
          };

          if (this.vendorId) {
            licUpdate.vendor_id = this.vendorId;
          }
          ...
          // Update by license_key
          const targetKey = this.centralizedKey || 'RJD-PISO-63C4DE5B-93AC2ADA';
          await this.supabase
            .from('licenses')
            .update(licUpdate)
      ...
      // 3. Update hardware_licenses table in Supabase (if present in schema)
      if (this.hardwareId) {
        try {
          await this.supabase
            .from('hardware_licenses')
            .update({
              daily_revenue: revenues.daily,
              weekly_revenue: revenues.weekly,
              monthly_revenue: revenues.monthly,
  ...
  async recordSale(saleData) {
    ...
    try {
      ...
      */
      ...
      // 1. Record sale in sales_logs table (using upsert with unique txId to prevent duplicate inserts)
      const rawTs = saleData.timestamp ? new Date(saleData.timestamp).getTime() : Date.now();
      const txId = saleData.id || saleData.transaction_id || `tx_${(this.hardwareId || 'hwd').replace(/[^a-zA-Z0-9]/g, '')}_${rawTs}_${saleData.amount}`;
      ...
      const { error } = await this.supabase
        ...
        .upsert({
          ...
          created_at: new Date(rawTs).toISOString(),
      ...
      try {
          ...
          const sessions = await db.all(`
            SELECT 
              ...
              wd.hostname, wd.custom_name, wd.subscription_type, wd.subscription_expires_at
            FROM sessions s
          ...
          for (const session of sessions) {
              ...
              updatesMap.set(sessionToken, {
                  ...
                  subscription_type: session.subscription_type || null,
          ...
          const filteredUpdates = updates.filter(update => {
              ...
              // Sync if remaining_seconds changed by more than 45 (keeps cloud within ~1 min of local burn)
              const remainingChanged = Math.abs((last.remaining_seconds || 0) - (update.remaining_seconds || 0)) > 45;
  ...
  async subscribeToRoaming() {
      ...
      // Prevent duplicate channels on resubscribe
      if (this._roamingChannel) {
          try { this.supabase.removeChannel(this._roamingChannel); } catch (e) {}
          this._roamingChannel = null;
      }

      console.log('[EdgeSync] Subscribing to roaming updates...');

      // '*' covers INSERT too: when another machine ADOPTS a session it often creates
      // a new row (or moves the token row) — we must see that hand-off immediately.
      const channel = this.supabase
        ...
        .subscribe((status) => {
            if (status === 'SUBSCRIBED') {
                console.log('[EdgeSync] Roaming realtime subscription active');
            } else if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT' || status === 'CLOSED') {
                console.warn(`[EdgeSync] Roaming realtime ${status} — resubscribing in 10s`);
                if (this._roamingReconnectTimer) return;
                this._roamingReconnectTimer = setTimeout(() => {
                    this._roamingReconnectTimer = null;
                    this.subscribeToRoaming();
                }, 10000);
                if (this._roamingReconnectTimer.unref) this._roamingReconnectTimer.unref();
            }
        });
      this._roamingChannel = channel;
  }
  
  /**
   * ARP-based presence check — is this MAC physically connected to THIS machine right now?
   * Used to decide roaming semantics: while we serve the device, our burn-down is truth;
   * when it leaves, we must relinquish the session so time isn't consumed twice.
   * Fail-safe: if ARP table is unreadable (non-Linux dev), report "present" so we never
   * kill an active session we can't verify.
   */
  _isMacPresent(mac) {
    try {
      if (!fs.existsSync('/proc/net/arp')) return true;
      const now = Date.now();
      if (this._arpPresenceCache && this._arpPresenceCacheTs && (now - this._arpPresenceCacheTs) < 5000) {
        return this._arpPresenceCache.has(String(mac).toUpperCase());
      }
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (cols.length >= 4 && cols[3] && cols[3] !== '00:00:00:00:00:00') set.add(cols[3].toUpperCase());
      }
      this._arpPresenceCache = set;
      this._arpPresenceCacheTs = now;
      return set.has(String(mac).toUpperCase());
    } catch (e) {
  ...
  /**
   * Apply a remote wifi_devices session row (from another machine) to local state.
   * Presence-aware last-write-wins:
   *  - Device present here  -> we are serving it; only adopt if remote grants MORE time (top-up elsewhere)
   *  - Device absent here   -> session moved elsewhere; relinquish (zero + clear token) so our
   *                            timer stops burning time the client is consuming on another board.
   *                            Clearing the token also makes future pushes use the fallback key so we
   *                            can never overwrite the cloud row now owned by the other machine.
   */
  async applyRemoteSessionRow(remoteDevice) {
    ...
    try {
      const mac = String(remoteDevice.mac_address).toUpperCase();
      const localSession = await db.get('SELECT remaining_seconds, total_paid, token, updated_at FROM sessions WHERE mac = ?', [mac]);
      if (!localSession) return; // no local session — restoration is probe-driven (checkRoamingForMac/Token)

      const localUpdated = new Date(localSession.updated_at || 0).getTime();
      const remoteUpdated = new Date(remoteDevice.updated_at || 0).getTime();
      if (!(remoteUpdated > localUpdated)) return; // local is newer — we are authoritative

      const remoteRemaining = Math.max(0, Number(remoteDevice.remaining_seconds) || 0);
      const present = this._isMacPresent(mac);

      // ─── Adopt subscription from cloud ───────────────────────────────
      if (remoteDevice.subscription_type) {
        const localDevice = await db.get('SELECT subscription_type, subscription_expires_at FROM wifi_devices WHERE mac = ?', [mac]);
        const localSub = localDevice?.subscription_type;
        const remoteSub = remoteDevice.subscription_type;
        
        // Adopt if remote has a subscription and local doesn't, or remote is newer
        if (remoteSub !== localSub || (remoteSub && remoteDevice.subscription_expires_at !== localDevice?.subscription_expires_at)) {
          await db.run(
            'UPDATE wifi_devices SET subscription_type = ?, subscription_expires_at = ? WHERE mac = ?',
            [remoteSub, remoteDevice.subscription_expires_at || null, mac]
          );
          console.log(`[EdgeSync] Roaming subscription adopted for ${mac}: ${remoteSub} (expires=${remoteDevice.subscription_expires_at || 'never'})`);
        }
      }

      if (present) {
        // Device is being served here: adopt only top-ups (more time), never downgrade
        if (remoteRemaining > (localSession.remaining_seconds || 0)) {
          console.log(`[EdgeSync] Roaming top-up for ${mac}: ${localSession.remaining_seconds}s -> ${remoteRemaining}s (applied from machine ${remoteDevice.machine_id})`);
          await db.run(
            'UPDATE sessions SET remaining_seconds = ?, total_paid = ?, updated_at = ?, expired_at = NULL WHERE mac = ?',
            [remoteRemaining, remoteDevice.total_paid || 0, new Date().toISOString(), mac]
          );
        }
        // else: our local burn-down is the truth while we serve the device
      } else {
        // Already relinquished? skip to avoid log spam / churn
        if ((localSession.remaining_seconds || 0) <= 0 && !localSession.token) return;
        console.log(`[EdgeSync] Roaming hand-off for ${mac}: session moved to machine ${remoteDevice.machine_id} — relinquishing local time`);
        await db.run(
          'UPDATE sessions SET remaining_seconds = 0, token = NULL, updated_at = ? WHERE mac = ?',
          [new Date().toISOString(), mac]
        );
      }
    } catch (e) {
      console.error('[EdgeSync] Error applying roaming session row:', e.message);
    }
  }

  async handleRoamingUpdate(remoteDevice) {
      await this.applyRemoteSessionRow(remoteDevice);
  }
  ...
  async syncRoamingSessions() {
      ...
      try {
           ...
           for (let i = 0; i < localMacs.length; i += BATCH_SIZE) {
               ...
               // NOTE: no remaining_seconds>0 filter — we MUST also see rows that hit 0
               // elsewhere, otherwise a client could spend all time on machine B and
               // still get free time when returning to this machine (double-spend).
               const { data: devices, error } = await this.supabase
                   .from('wifi_devices')
                   .select('mac_address, machine_id, remaining_seconds, total_paid, updated_at')
                   .eq('vendor_id', this.vendorId)
               ...
               if (devices && devices.length > 0) {
                   for (const dev of devices) {
                       // Shared presence-aware logic (newest rows apply first;
                       // once local updated_at is bumped, older rows are skipped)
                       await this.applyRemoteSessionRow(dev);
                   }
               }
           }
      } catch (e) {
  ...
  async checkRoamingForMac(mac) {
      ...
      try {
          const { data: rows, error } = await this.supabase
            .from('wifi_devices')
            .select('mac_address, session_token, remaining_seconds, total_paid, updated_at')
            .eq('vendor_id', this.vendorId)
            .eq('mac_address', mac)
          ...
          if (data && data.remaining_seconds > 0) {
              ...
              if (existing) {
                  ...
                  await db.run(
                      'UPDATE sessions SET remaining_seconds = ?, total_paid = ?, token = ?, updated_at = ?, expired_at = CASE WHEN ? > 0 THEN NULL ELSE expired_at END WHERE mac = ?',
                      [data.remaining_seconds, data.total_paid || 0, data.session_token || null, new Date().toISOString(), data.remaining_seconds, mac]
                  );
              } else {
                   ...
                   try {
                       await db.run(
                           'INSERT INTO sessions (mac, remaining_seconds, total_paid, connected_at, updated_at, token, is_paused) VALUES (?, ?, ?, ?, ?, ?, 0)',
                           [mac, data.remaining_seconds, data.total_paid || 0, new Date().toISOString(), new Date().toISOString(), data.session_token || null]
                       );
                   } catch (insertErr) {
              ...
              // OWNERSHIP STAMP: immediately push this session to cloud with OUR machine_id.
              // The upsert-by-session_token moves the cloud row to this machine, so the
              // ORIGINAL machine sees the hand-off in realtime and relinquishes its copy
              // (prevents both machines burning the same time).
              this.syncDeviceToCloud(mac, data.remaining_seconds, data.total_paid || 0)
                  .catch(e => console.error('[EdgeSync] Failed to stamp roaming adoption:', e.message));
  ...
  async checkRoamingForToken(sessionToken, mac, ipAddress = null) {
      ...
      try {
          ...
          if (existing) {
              ...
              await db.run(
                  'UPDATE sessions SET ip = COALESCE(?, ip), remaining_seconds = ?, total_paid = ?, token = ?, updated_at = ?, expired_at = CASE WHEN ? > 0 THEN NULL ELSE expired_at END WHERE mac = ?',
                  [ipAddress, data.remaining_seconds, data.total_paid || 0, sessionToken, nowIso, data.remaining_seconds, finalMac]
              );
          } else {
              try {
                  await db.run(
          ...
          // OWNERSHIP STAMP: move the cloud session row to this machine immediately
          this.syncDeviceToCloud(finalMac, data.remaining_seconds, data.total_paid || 0)
              .catch(e => console.error('[EdgeSync] Failed to stamp roaming adoption (token):', e.message));
  ...
  async syncDeviceToCloud(mac, remainingSeconds, totalPaid = 0) {
      ...
      try {
          ...
          // Get subscription info from wifi_devices
          const device = await db.get('SELECT subscription_type, subscription_expires_at FROM wifi_devices WHERE mac = ?', [mac]);
          ...
          const updatePayload = {
              ...
              subscription_type: device?.subscription_type || null,
              subscription_expires_at: device?.subscription_expires_at || null,
              updated_at: new Date().toISOString()
          };
  ...
  /**
   ...
   async checkPendingCommands() {
       if (!this.supabase || !this.machineId || !this.machineCommandsSupported) return;
       ...
       try {
           ...
           if (error) {
               if (error.message && (error.message.includes('machine_commands') || error.message.includes('schema cache') || error.code === 'PGRST205' || error.code === '42P01')) {
                   this.machineCommandsSupported = false;
                   return;
               }
