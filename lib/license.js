...
class LicenseManager {
  ...
  deriveCategory(licenseKey) {
    if (!licenseKey) return 'piso_wifi';
    const key = String(licenseKey).toUpperCase().trim();
    if (key.startsWith('RJD-RENT-')) return 'phone_rental';
    if (key.startsWith('RJD-MCU-')) return 'nodemcu';
    if (key.startsWith('RJD-MIKRO-')) return 'mikrotik';
    if (key.startsWith('RJD-PISO-') || key.startsWith('RJD-')) return 'piso_wifi';
    return 'piso_wifi';
  }

  getFeaturesForCategory(category) {
    const cat = category || 'piso_wifi';
    return {
      piso_wifi: cat === 'piso_wifi' || cat === 'all',
      phone_rental: cat === 'phone_rental' || cat === 'all',
      nodemcu: cat === 'nodemcu' || cat === 'all',
      mikrotik: cat === 'mikrotik' || cat === 'all'
    };
  }

  /**
   * Verify if the current device has a valid license
   * @returns {Promise<{isValid: boolean, isActivated: boolean, category?: string, features?: object, error?: string, licenseKey?: string, expiresAt?: Date}>} License verification status
   */
  async verifyLicense() {
    if (!this.supabase) {
      ...
      try {
        ...
        if (localLicense) {
          ...
          const cat = this.deriveCategory(localLicense.license_key);
          return { 
            isValid: true, 
            isActivated: true,
            ...
            category: cat,
            features: this.getFeaturesForCategory(cat),
          ...
          return { 
            ...
            category: 'none',
            features: this.getFeaturesForCategory('none'),
    ...
    try {
      ...
      if (!license.is_active) {
        ...
        await db.run('UPDATE license_info SET is_active = 0, is_revoked = 1 WHERE hardware_id = ?', [hardwareId]);
        const cat = this.deriveCategory(license.license_key);
        return { 
          isValid: false, 
          isActivated: false, 
          isRevoked: true,
