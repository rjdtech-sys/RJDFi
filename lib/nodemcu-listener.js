...
class NodeMCUListener {
  ...
  setupRoutes() {
    ...
    this.app.post('/api/nodemcu/pulse', express.json(), async (req, res) => {
      try {
        ...
        const updatedDevice = { ...device, totalPulses: (device.totalPulses || 0) + denomination, totalRevenue: (device.totalRevenue || 0) + denomination, lastSeen: now };
    ...
    this.socketIo.on('connection', (socket) => {
      ...
      socket.on('authenticate', async (data, callback) => {
        try {
          ...
          // Update device status and last seen in table
          const now = new Date().toISOString();
          await db.updateNodeMCUDevice(macAddress, { lastSeen: now, status: 'connected' });
  ...
  async updateDeviceStatus(macAddress, status) {
    try {
      const now = new Date().toISOString();
      await db.updateNodeMCUDevice(macAddress, { status, lastSeen: now });
