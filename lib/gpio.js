...
async function initGPIO(
  ...
  if (Gpio && sysPin !== -1) {
    try {
      ...
      try {
        if (hasEdge) {
          coinInput = new Gpio(sysPin, 'in', 'both', { debounceTimeout: 20 });
          console.log(`[GPIO] Using interrupt mode (both edges)`);
        } else {
        ...
        if (edgeErr.message && (edgeErr.message.includes('edge') || edgeErr.message.includes('ENOENT'))) {
          ...
          coinInput = new Gpio(sysPin, 'in', 'none', { debounceTimeout: 20 });
          usePolling = true;
      ...
      const MIN_GPIO_PULSE_INTERVAL = 35; // 35ms debouncing for fast/slow coinslots
      ...
      if (usePolling) {
        ...
        function schedulePoll() {
          ...
          const tid = setTimeout(async () => {
            try {
              if (!coinInput || _pollStopped) return;
        ...
        _gpioPaused = false;
        console.log(`[GPIO] SUCCESS: GPIO ${sysPin} initialized (adaptive polling) — ACTIVE, listening for coin pulses.`);
        schedulePoll();
      } else {
        ...
        _coinWatchCallback = (err, value) => {
          ...
          const now = Date.now();
          if (now - lastGPIOPulseTime < MIN_GPIO_PULSE_INTERVAL) {
            return; // Skip duplicate edge/noise
          }
          
          pulseCount++;
          lastGPIOPulseTime = now;
          console.log(`[GPIO] Pulse interrupt detected on GPIO ${sysPin} (val=${value}, Count: ${pulseCount})`);
        ...
        _gpioPaused = false;
        console.log(`[GPIO] SUCCESS: GPIO ${sysPin} initialized (interrupt mode) — ACTIVE 24/7, listening for coin pulses.`);
      }
    } catch (e) {
...
/**
 * Pause GPIO coin detection — stops polling loop if in polling mode.
 * For hardware interrupt mode on Linux (OPi/RPi), interrupts stay active 24/7 (0% CPU).
 */
function pauseGPIO() {
  if (_gpioPaused) return;
  if (_gpioUsePolling) {
    _gpioPaused = true;
    if (coinInput && coinInput.__pollTimeout) {
      clearTimeout(coinInput.__pollTimeout);
      coinInput.__pollTimeout = null;
    }
    console.log('[GPIO] Polling mode paused');
  }
}

/**
 * Resume GPIO coin detection — restarts polling loop if in polling mode.
 */
function resumeGPIO() {
  if (!_gpioPaused) return;
  _gpioPaused = false;
  
  if (_gpioUsePolling && coinInput && _schedulePollFn) {
    _pollStopped = false;
    _schedulePollFn();
    console.log('[GPIO] Polling mode resumed');
  }
}
