...
// On-demand GPIO pause/resume state
let _gpioPaused = false;
let _gpioUsePolling = false;
let _pollStopped = false;
let _schedulePollFn = null;  // reference to schedulePoll function for resume
let _coinWatchCallback = null; // reference to watch callback for resume
...
async function initGPIO(
  ...
  // Cleanup previous GPIO
  if (coinInput) {
    try {
      // Clear polling if in adaptive polling mode
      if (coinInput.__pollTimeout) {
        clearTimeout(coinInput.__pollTimeout);
        coinInput.__pollTimeout = null;
      }
  ...
  // x64_pc has no local GPIO — NodeMCU devices handle coin detection
  if (boardType === 'x64_pc') {
    console.log(`[GPIO] x64 PC Mode — No local GPIO. Coin detection via NodeMCU devices only.`);
    return;
  }

  // Legacy 'none' board type — treat as x64_pc (no GPIO)
  if (boardType === 'none') {
    console.log(`[GPIO] Legacy 'none' board type detected — treating as no-GPIO mode.`);
    return;
  }
  ...
  if (Gpio && sysPin !== -1) {
    try {
      ...
      if (usePolling) {
        // Adaptive polling: slow when idle, fast only during active pulse
        // Reduces CPU load by ~90% vs constant 50Hz while still detecting ~50ms coin pulses
        let lastValue = 0;
        let consecutiveErrorCount = 0;
        let isPulseActive = false;
        let pulseEndTimeout = null;
        _pollStopped = false;
        
        const IDLE_INTERVAL = 100;   // 10Hz when idle — minimal CPU
        const ACTIVE_INTERVAL = 10;  // 100Hz during pulse — catches every ~50ms pulse
        
        function schedulePoll() {
          if (_pollStopped || _gpioPaused || !coinInput) return;
          const delay = isPulseActive ? ACTIVE_INTERVAL : IDLE_INTERVAL;
          const tid = setTimeout(async () => {
            try {
              if (!coinInput || pollStopped) return;
              ...
              if (val === 1 && lastValue === 0) {
                ...
                if (now - lastGPIOPulseTime >= MIN_GPIO_PULSE_INTERVAL) {
                  ...
                  if (pulseEndTimeout) clearTimeout(pulseEndTimeout);
              ...
              // When pin goes low after being high, pulse is ending
              if (val === 0 && isPulseActive) {
                if (pulseEndTimeout) clearTimeout(pulseEndTimeout);
                pulseEndTimeout = setTimeout(() => {
                  isPulseActive = false;
                }, 150);
              }
          ...
          coinInput.__pollTimeout = tid;
          if (tid.unref) tid.unref(); // Don't let polling keep the event loop alive
        }
        
        // Store reference for pause/resume
        _schedulePollFn = schedulePoll;
        _gpioUsePolling = true;
        
        // Start paused — GPIO only activates when INSERT COIN is pressed
        _gpioPaused = true;
        console.log(`[GPIO] SUCCESS: GPIO ${sysPin} initialized (adaptive polling) — PAUSED, waiting for INSERT COIN.`);
      } else {
        // Standard interrupt-based watching
        _coinWatchCallback = (err, value) => {
          ...
          pulseCount++;
          lastGPIOPulseTime = now;
          console.log(`[GPIO] Pulse interrupt detected on GPIO ${sysPin} (Count: ${pulseCount})`);

          if (pulseTimer) clearTimeout(pulseTimer);
          pulseTimer = setTimeout(() => {
            console.log(`[GPIO] Processing ${pulseCount} pulse(s) for Main Coinslot`);
            handlePulses(pulseCount);
            pulseCount = 0;
          }, 250);
        };
        
        coinInput.watch(_coinWatchCallback);
        _gpioUsePolling = false;
        _gpioPaused = false;
        console.log(`[GPIO] SUCCESS: GPIO ${sysPin} initialized (interrupt mode) — ACTIVE, listening for coin pulses.`);
      }
    } catch (e) {
...
async function updateGPIO(
  ...
  const pinLabel = (boardType === 'x64_pc' || boardType === 'none') ? 'No GPIO' : `Pin ${pin}`;
  console.log(`[HARDWARE] Reconfiguring: ${boardType} (${boardModel || 'Generic'}), ${pinLabel}`);
  if (boardType === 'nodemcu_esp') {
...
/**
 * Pause GPIO coin detection — stops polling/interrupts to save CPU.
 * Called when no portal client is waiting for coin insertion.
 */
function pauseGPIO() {
  if (_gpioPaused) return; // Already paused
  _gpioPaused = true;
  ...
  if (coinInput) {
    if (_gpioUsePolling) {
      ...
      // Disable interrupt watching
      try { coinInput.disable(); } catch (_e) {}
    }
  }
  console.log('[GPIO] Paused — coin detection inactive (saves CPU)');
}

/**
 * Resume GPIO coin detection — restarts polling/interrupts.
 * Called when a portal client presses INSERT COIN.
 */
function resumeGPIO() {
  if (!_gpioPaused) return; // Already active
  _gpioPaused = false;
  
  if (coinInput) {
    if (_gpioUsePolling && _schedulePollFn) {
      // Restart the adaptive polling loop
      _pollStopped = false;
      _schedulePollFn();
    } else if (!_gpioUsePolling) {
      // Re-enable interrupt watching
      try { coinInput.enable(); } catch (_e) {}
    }
  }
  console.log('[GPIO] Resumed — coin detection active, listening for pulses');
}

/**
 * Check if GPIO is currently paused.
 */
function isGPIOPaused() {
  return _gpioPaused;
}

module.exports = { initGPIO, updateGPIO, registerSlotCallback, unregisterSlotCallback, setRelayState, pauseGPIO, resumeGPIO, isGPIOPaused };
