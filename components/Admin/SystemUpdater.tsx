...
const SystemUpdater: React.FC = () => {
  ...
  const handleGitUpdate = async () => {
    if (!confirm('Pull latest code directly from official GitHub repository? The system will update and restart automatically.')) return;
    ...
    try {
      ...
      const res = await fetch('/api/system/git-update', { method: 'POST', headers });
      const data = await res.json();

      if (res.ok) {
        alert('✅ System updated from GitHub successfully! The system will restart in 3 seconds.');
        setTimeout(() => window.location.reload(), 3000);
      } else {
        throw new Error(data.error || 'Git update failed');
      }
    } catch (error: any) {
      alert('❌ Error updating from GitHub: ' + error.message);
    } finally {
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap gap-3">
            <button
              ...
              disabled={isScanningUpdate || isGitUpdating}
            ...
            <button
              ...
              disabled={isGitUpdating || isScanningUpdate}
              className="bg-emerald-600 text-white px-6 py-2 rounded-lg font-bold text-sm uppercase tracking-wide hover:bg-emerald-700 transition-all shadow-md disabled:opacity-50 flex items-center gap-2"
            >
              ...
              {isGitUpdating ? 'Updating from GitHub...' : 'Update From GitHub'}
            </button>
          </div>
