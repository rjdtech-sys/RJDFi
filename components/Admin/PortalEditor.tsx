          ...
          >
            ...
            <div
              className={`pt-10 pb-12 px-4 text-center shadow-lg relative ${
                ...
                config.theme === 'aircoins' ? 'rounded-b-[28px] border-b' :
                'rounded-b-[20px]'
              }`}
              style={{
                background: config.theme === 'cyberpunk'
                  ? 'linear-gradient(135deg, rgba(26,26,46,0.92) 0%, rgba(22,33,62,0.88) 100%)'
                  : config.theme === 'aircoins'
                  ? 'linear-gradient(135deg, #101422 0%, #1C2538 100%)'
                ...
                borderColor: config.theme === 'cyberpunk' ? config.primaryColor
                  : config.theme === 'retro-pixel' ? config.secondaryColor
                  : config.theme === 'aircoins' ? '#1E2638' : undefined,
            ...
            <div className="flex-1 px-3 -mt-6 relative z-10">
              <div
                className={`p-4 shadow-xl border text-center ${
                  ...
                  config.theme === 'aircoins' ? 'rounded-[16px] border backdrop-blur-md' :
                  'rounded-[20px] bg-white/90 backdrop-blur-sm border-white/20'
                }`}
                style={{
                  color: ['cyberpunk', 'gaming', 'neon-arena', 'space-wars', 'retro-pixel', 'dragon-fire', 'aircoins'].includes(config.theme) ? '#f0f9ff' : '#0f172a',
                  ...(config.theme === 'aircoins' ? { backgroundColor: '#151A28', borderColor: '#1E2638' } : {})
                }}
              >
                ...
                <h2 className={`text-3xl font-black mb-3 tracking-tighter ${
                  ['gaming', 'cyberpunk', 'neon-arena', 'space-wars', 'dragon-fire', 'aircoins'].includes(config.theme) ? `drop-shadow-[0_0_12px_${config.primaryColor}]` : ''
                }`}>00:00:00</h2>
                
                <div className="flex justify-center gap-2 mb-4">
                   <div className="h-1.5 w-1.5 rounded-full bg-green-500"></div>
                   <span className="text-[7px] font-black uppercase tracking-widest" style={{ color: ['cyberpunk','gaming','neon-arena','space-wars','retro-pixel','dragon-fire','aircoins'].includes(config.theme) ? '#94a3b8' : '#64748b' }}>System Ready</span>
                </div>

                <div className="space-y-2">
                  <button
                    className={`w-full py-2 font-black text-[9px] uppercase tracking-widest text-white shadow-md ${
                      config.theme === 'gaming' || config.theme === 'cyberpunk' || config.theme === 'neon-arena' || config.theme === 'space-wars' || config.theme === 'dragon-fire' || config.theme === 'aircoins' ? 'rounded-full' :
                ...
                {[1, 5].map((amt) => (
                   <div
                     ...
                     className={`p-2 text-center border ${
                       ...
                       config.theme === 'aircoins' ? 'rounded-xl border backdrop-blur-sm' :
                       'rounded-xl bg-white/80 backdrop-blur-sm border-slate-100'
                     }`}
                     style={config.theme === 'aircoins' ? { backgroundColor: '#1C2234', borderColor: '#1E2638' } : undefined}
                   >
                      <span className={`block text-sm font-black ${['cyberpunk','gaming','neon-arena','space-wars','retro-pixel','dragon-fire','aircoins'].includes(config.theme) ? 'text-white' : 'text-slate-900'}`}>₱{amt}</span>
