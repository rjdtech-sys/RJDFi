...
const NetworkSettings: React.FC = () => {
  ...
  // Anti-Tethering State
  const [antiTethering, setAntiTethering] = useState({
    installed: false,
    enabled: false,
    ttl_value: 64,
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [ifaces, hs, wifi, v, b, atStatus] = await Promise.all([
        ...
        apiClient.getAntiTetheringStatus().catch(() => ({ installed: false, enabled: false, ttl_value: 64, interfaces: [] }))
      ]);
      ...
      setAntiTethering(atStatus || { installed: false, enabled: false, ttl_value: 64, interfaces: [] });
      // Pre-select interfaces from saved config or all hotspot interfaces
      if (atStatus && atStatus.interfaces && atStatus.interfaces.length > 0) {
        setSelectedIfaces(atStatus.interfaces);
      } else if (Array.isArray(hs) && hs.length > 0) {
        setSelectedIfaces(hs.map((h: any) => h.interface));
      }
      if (atStatus && atStatus.ttl_value) {
        setTtlInput(atStatus.ttl_value);
      }
    } catch (err) { 
  ...
  const deployBridge = async () => {
    if (!bridge.name) return alert('Bridge name is required!');
    try {
      setLoading(true);
      await apiClient.createBridge(bridge.name, bridge.members, bridge.stp);
      await loadData();
      alert(bridge.members.length > 0
        ? `Bridge ${bridge.name} created! Members have been flushed to prevent IP conflicts.`
        : `Bridge ${bridge.name} created with no member interfaces. You can add members later.`);
  ...
  // Anti-Tethering Handlers
  const toggleAntiTetherIface = (iface: string) => {
    setSelectedIfaces(prev =>
      prev.includes(iface) ? prev.filter(i => i !== iface) : [...prev, iface]
    );
  };

  const applyAntiTethering = async () => {
    if (selectedIfaces.length === 0) return alert('Select at least one portal segment!');
    if (!Number.isInteger(ttlInput) || ttlInput < 1 || ttlInput > 255) return alert('TTL must be between 1 and 255!');
    try {
      setLoading(true);
      await apiClient.applyAntiTethering(ttlInput, selectedIfaces);
      await loadData();
      alert('Anti-Tethering rules applied successfully!');
    } catch (e: any) {
      alert('Failed to apply Anti-Tethering: ' + (e.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };

  const removeAntiTethering = async () => {
    if (!confirm('Remove all Anti-Tethering rules?')) return;
    try {
      setLoading(true);
      await apiClient.removeAntiTethering();
      await loadData();
      alert('Anti-Tethering rules removed.');
    } catch (e: any) {
      alert('Failed to remove Anti-Tethering: ' + (e.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };

  const installIptablesPersistent = async () => {
    try {
      setLoading(true);
      await apiClient.installIptablesPersistent();
      await loadData();
      alert('iptables-persistent installed successfully!');
    } catch (e: any) {
      alert('Failed to install iptables-persistent: ' + (e.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4 pb-20 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      {/* 3.5 Anti-Tethering Engine */}
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-2">
            <span className="text-sm">🛡️</span>
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Anti-Tethering</h3>
            {antiTethering.enabled && (
              <span className="bg-green-100 text-green-700 text-[7px] font-black px-1.5 py-0.5 rounded uppercase">Active</span>
            )}
            {!antiTethering.installed && (
              <span className="bg-yellow-100 text-yellow-700 text-[7px] font-black px-1.5 py-0.5 rounded uppercase">Not Installed</span>
            )}
          </div>
          {antiTethering.enabled && (
            <button onClick={removeAntiTethering} disabled={loading} className="text-[9px] font-black uppercase text-red-600 hover:text-red-700 disabled:opacity-50">
              Remove Rules
            </button>
          )}
        </div>
        <div className="p-4 space-y-4">
          ...
          {!antiTethering.installed && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-center justify-between">
              <div>
                <p className="text-[10px] font-black text-yellow-800 uppercase">iptables-persistent Required</p>
                <p className="text-[8px] text-yellow-600 font-bold mt-0.5">This package saves firewall rules across reboots. Install it to persist Anti-Tethering rules.</p>
              </div>
              <button
                onClick={installIptablesPersistent}
                disabled={loading}
                className="bg-yellow-600 hover:bg-yellow-700 text-white px-3 py-1.5 rounded-lg font-black text-[8px] uppercase tracking-widest disabled:opacity-50 whitespace-nowrap ml-3"
              >
          ...
          {/* Portal Segments Selection */}
          <div>
            <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Active Portal Segments</label>
            {hotspots.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                {hotspots.map((hs: any) => (
                  <button
                    key={hs.interface}
                    onClick={() => toggleAntiTetherIface(hs.interface)}
                    className={`py-2 px-3 rounded-lg border text-[9px] font-black uppercase transition-all flex items-center gap-2 ${
                      selectedIfaces.includes(hs.interface)
                        ? 'bg-blue-600 border-blue-600 text-white'
                        : 'bg-white border-slate-200 text-slate-500 hover:border-blue-300'
                    }`}
                  >
                    <span className={`w-3 h-3 rounded border flex items-center justify-center text-[7px] ${
                      selectedIfaces.includes(hs.interface) ? 'bg-white/20 border-white/40' : 'bg-slate-100 border-slate-300'
                    }`}>
                      {selectedIfaces.includes(hs.interface) ? '✓' : ''}
                    </span>
                    {hs.interface}
              ...
              <div className="py-6 text-center border-2 border-dashed border-slate-200 rounded-lg text-slate-300 text-[9px] font-black uppercase">
                No Active Portal Segments — Create one above first
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 block">TTL Value</label>
              <input
                type="number"
                min={1}
                max={255}
                value={ttlInput}
                onChange={e => setTtlInput(parseInt(e.target.value) || 64)}
                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs font-mono font-bold"
              />
              <p className="text-[7px] text-slate-400 mt-1">Default: 64. Use 1 for strict anti-tethering (packets expire after 1 hop).</p>
            </div>
            <div className="flex items-end">
              <button
                onClick={applyAntiTethering}
                disabled={loading || !antiTethering.installed || hotspots.length === 0}
                className="admin-btn-primary w-full py-2 rounded-lg font-black text-[9px] uppercase tracking-widest shadow-md disabled:opacity-50"
              >
                {loading ? 'Applying...' : antiTethering.enabled ? 'Update Rules' : 'Apply'}
              </button>
            </div>
          </div>
          ...
          {antiTethering.enabled && (
            <div className="bg-slate-50 rounded-lg p-3 border border-slate-100">
              <p className="text-[8px] font-black text-slate-500 uppercase mb-1">Current Configuration</p>
              <div className="flex flex-wrap gap-3 text-[9px] font-mono text-slate-700">
                <span>TTL: <strong>{antiTethering.ttl_value}</strong></span>
                <span>Interfaces: <strong>{antiTethering.interfaces.join(', ') || 'None'}</strong></span>
              </div>
            </div>
          )}
        </div>
