...
const WanTrafficMonitor: React.FC<{ wans: WanInterface[] }> = ({ wans }) => {
    ...
    const fetchTraffic = async () => {
      try {
        ...
        setTrafficHistory(prev => {
          ...
          for (const [iface, stats] of Object.entries(data) as [string, WanTrafficData][]) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Theme-aware colors
      const isDark = document.documentElement.getAttribute('data-theme') === 'aircoins';

      // Clear canvas
      ctx.fillStyle = isDark ? '#0F121D' : '#f8fafc';
      ctx.fillRect(0, 0, width, height);

      // Draw grid
      ctx.strokeStyle = isDark ? '#1E2638' : '#e2e8f0';
      ctx.lineWidth = 1;
      ...
      // Draw axis labels
      ctx.fillStyle = isDark ? '#8A94A6' : '#94a3b8';
      ...
      let maxRate = 125000; // Default 1 Mbps minimum scale (125000 bytes/s = 1 Mbps)
      ...
      // Colors for each WAN
      const colors = isDark ? [
        { rx: '#38bdf8', tx: '#7dd3fc' }, // Cyan-blue
        { rx: '#4ade80', tx: '#86efac' }, // Green
        { rx: '#a78bfa', tx: '#c4b5fd' }, // Purple
        { rx: '#fbbf24', tx: '#fcd34d' }, // Amber
      ] : [
      ...
      wans.forEach((wan, wanIndex) => {
        ...
        // Current rate
        if (stats) {
          ctx.fillStyle = isDark ? '#8A94A6' : '#64748b';
  ...
  const formatRate = (bytesPerSec: number): string => {
    ...
    if (bitsPerSec < 1000000000) return `${(bitsPerSec / 1000000).toFixed(2)} Mbps`;
...
const MultiWanSettings: React.FC = () => {
  ...
  const [healthData, setHealthData] = useState<Record<string, WanHealthData>>({});
  ...
  // Configure system WAN form (separate from Add)
  const [configureForm, setConfigureForm] = useState<Partial<WanInterface>>({
    name: '',
    type: 'dhcp',
    config: {},
    gateway: '',
  ...
  useEffect(() => {
    ...
    // Socket.IO for real-time Multi-WAN health data
    const socket = io();
    socketRef.current = socket;
    socket.on('multiwan-health', (data: Record<string, WanHealthData>) => {
      setHealthData(data);
    });
  ...
  const handleConfigureDefaultWan = () => {
    ...
    const existingWan = wans.find(w => w.name === defaultWan);

    if (existingWan) {
      // Already in DB — open the Edit modal directly with existing data
      setEditingWan({ ...existingWan });
      setShowEditModal(true);
    } else {
      // System WAN not in DB — open Configure modal pre-filled with detected data
      setConfigureForm({
        name: defaultWan,
        type: 'dhcp',
        config: {},
  ...
  const handleConfigureSave = async () => {
    if (!configureForm.name) return;

    // Check if a WAN entry already exists for this interface
    const existingWan = wans.find(w => w.name === configureForm.name);
    if (existingWan) {
      // Update existing DB entry
      try {
        await apiClient.updateWanInterface(existingWan.id!, {
          name: configureForm.name,
          type: configureForm.type,
          config: configureForm.config,
          gateway: configureForm.gateway,
          weight: configureForm.weight,
          enabled: configureForm.enabled
        });
        setShowConfigureModal(false);
        setConfigureForm({ name: '', type: 'dhcp', config: {}, gateway: '', weight: 1, enabled: 1 });
        fetchWans();
      } catch (e: any) {
        alert('Failed to update WAN: ' + e.message);
      }
    } else {
      // Create new DB entry for the system WAN
      const ifaceName = configureForm.name;
      const selectedIface = availableInterfaces.find(i => i.name === ifaceName);
      const isVlan = selectedIface?.type === 'vlan' || (ifaceName.includes('.') && !selectedIface) ? 1 : 0;
      let vlan_parent = null;
      let vlan_id = null;
      ...
      try {
        await apiClient.createWanInterface({
          name: ifaceName,
          type: configureForm.type!,
          config: configureForm.config || {},
          gateway: configureForm.gateway || null,
          weight: configureForm.weight || 1,
          enabled: configureForm.enabled ?? 1,
          is_vlan: isVlan,
          vlan_parent,
          vlan_id
        });
        ...
        alert('Failed to configure WAN: ' + e.message);
      }
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-2 duration-500">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {config.topology === 'multi' && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                <div>
                  <h3 className="font-black text-slate-800 uppercase tracking-widest text-sm">ECMP Load Balancing</h3>
                  <p className="text-[9px] text-slate-400 font-bold uppercase mt-0.5">Kernel-level per-flow hashing across all active WANs</p>
                </div>
              ...
              <div className="p-6 space-y-6">
                {/* WAN Health Status */}
                {config.enabled && wans.filter(w => !!w.enabled).length > 0 && (
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">WAN Health Monitor</label>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {wans.filter(w => !!w.enabled).map(wan => {
                        const h = healthData[wan.name];
                        const isAlive = h ? h.alive : (wan.status === 'up');
                        const isChecking = !h;

                        return (
                          <div key={wan.id} className={`p-4 rounded-xl border-2 transition-all ${isAlive ? 'border-emerald-200 bg-emerald-50/50' : 'border-red-200 bg-red-50/50'}`}>
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2">
                                <span className={`inline-block w-2.5 h-2.5 rounded-full ${isAlive ? 'bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.5)]' : 'bg-red-500 shadow-[0_0_6px_rgba(239,68,68,0.5)]'}`} />
                                <span className="font-black text-sm text-slate-800 uppercase">{wan.name}</span>
                              </div>
                              <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${isAlive ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                                {isChecking ? 'CHECKING...' : isAlive ? 'ALIVE' : 'DEAD'}
                              </span>
                            </div>
                            {h ? (
                              <div className="grid grid-cols-3 gap-2 text-center">
                                <div>
                                  <div className="text-[8px] font-black text-slate-400 uppercase">Ping</div>
                                  <div className="text-sm font-black text-slate-700">{h.pingMs !== null ? `${h.pingMs}ms` : '—'}</div>
                                </div>
                                <div>
                                  <div className="text-[8px] font-black text-slate-400 uppercase">Targets</div>
                                  <div className="text-sm font-black text-slate-700">{h.targetsHit}/{h.totalTargets}</div>
                                </div>
                                <div>
                                  <div className="text-[8px] font-black text-slate-400 uppercase">Fails</div>
                                  <div className={`text-sm font-black ${h.failCount > 0 ? 'text-red-600' : 'text-slate-700'}`}>{h.failCount}</div>
                                </div>
                              </div>
                            ) : (
                              <div className="text-[10px] text-slate-400 text-center py-1">Waiting for health data...</div>
                            )}
                          </div>
                        );
                      })}
                ...
                {config.enabled && wans.filter(w => !!w.enabled).length > 0 && (
                  <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <div className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Combined Bandwidth</div>
                        <div className="text-[9px] text-blue-500 font-bold mt-0.5">Total available from all active WANs</div>
                      </div>
                      <div className="text-right">
                        <div className="text-2xl font-black text-blue-700">
                          {(() => {
                            const aliveWans = wans.filter(w => {
                              const h = healthData[w.name];
                              return w.enabled && (h ? h.alive : w.status === 'up');
                            });
                            const totalSpeed = aliveWans.reduce((sum, wan) => {
                              const iface = availableInterfaces.find(i => i.name === wan.name);
                              return sum + (iface?.speed || 0);
                            }, 0);
                            return totalSpeed > 0 ? formatSpeed(totalSpeed) : '—';
                          })()}
                        </div>
                        <div className="text-[9px] font-bold text-blue-500">
                          {(() => {
                            ...
                            return `${aliveWans.length} active WAN${aliveWans.length !== 1 ? 's' : ''}`;
                          })()}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white/60 rounded-lg p-3 border border-blue-100">
                      <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-2">ECMP Pool</div>
                      <div className="space-y-1.5">
                        {wans.filter(w => !!w.enabled).map(wan => {
                          ...
                          const weight = wan.weight || 1;
                          
                          return (
                            <div key={wan.id} className="flex items-center justify-between text-xs">
                              <div className="flex items-center gap-2">
                                <span className={`inline-block w-2 h-2 rounded-full ${isAlive ? 'bg-emerald-500' : 'bg-red-500'}`} />
                                <span className="font-bold text-slate-700">{wan.name}</span>
                                <span className="text-[9px] text-slate-500">(w{weight})</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <span className="text-[9px] font-bold text-slate-600">{formatSpeed(speed) || '—'}</span>
                                <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded ${isAlive ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                                  {isAlive ? 'IN POOL' : 'REMOVED'}
                                </span>
                              </div>
                            </div>
                          );
                      ...
                      <div className="mt-2 pt-2 border-t border-blue-100">
                        <div className="text-[8px] text-slate-500 leading-relaxed">
                          <strong>How ECMP works:</strong> Traffic is distributed per-connection using kernel-level hashing. 
                          A single download uses one WAN, but multiple simultaneous connections are spread across all active WANs. 
                          Weights control the distribution ratio (e.g., w2 gets 2x the traffic of w1).
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-indigo-600 rounded-2xl p-6 text-white shadow-xl shadow-indigo-600/10">
            ...
            <div className="space-y-4 text-xs leading-relaxed opacity-90">
              ...
              <p>
                ...
                ECMP distributes traffic across multiple WANs using kernel-level per-flow hashing. Requires 2+ active WANs.
              </p>
              <p>
                <strong className="block text-indigo-200 mb-1 uppercase text-[10px]">Health Monitoring</strong>
                Each WAN is ping-checked every 5 seconds against 3 targets. Dead WANs are automatically removed from the pool.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Configure System WAN Modal */}
      {showConfigureModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-slate-100">
              <h3 className="font-black text-slate-800 uppercase tracking-widest text-sm">Configure WAN: {configureForm.name}</h3>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Interface</label>
                <div className="w-full p-2 rounded-lg border border-slate-200 text-sm font-bold text-slate-500 bg-slate-50 font-mono">
                  {configureForm.name}
                </div>
              </div>

              {renderConfigFields(configureForm, setConfigureForm)}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="192.168.1.1"
                    className="w-full p-2 rounded-lg border border-slate-200 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono"
                    value={configureForm.gateway || ''}
                    onChange={e => setConfigureForm({ ...configureForm, gateway: e.target.value })}
                  />
                </div>
                <div>
                  <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Weight</label>
                  <input
                    ...
                    max="100"
                    className="w-full p-2 rounded-lg border border-slate-200 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    value={configureForm.weight || 1}
                    onChange={e => setConfigureForm({ ...configureForm, weight: parseInt(e.target.value) || 1 })}
                  />
                </div>
              </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={e => setConfigureForm({ ...configureForm, enabled: e.target.checked ? 1 : 0 })}
                  className="rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                />
            ...
            <div className="p-6 border-t border-slate-100 flex justify-end gap-2">
              <button onClick={() => setShowConfigureModal(false)} className="px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-colors">Cancel</button>
              <button onClick={handleConfigureSave} className="px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest bg-blue-600 text-white hover:bg-blue-700 transition-colors">Save</button>
            </div>
          </div>
        </div>
      )}
